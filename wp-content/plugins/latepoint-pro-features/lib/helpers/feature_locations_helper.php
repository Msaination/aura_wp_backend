...
class OsFeatureLocationsHelper {
	...
	public static function load_step_locations( $step_code, $format = 'json' ) {
		if ( $step_code == 'booking__locations' ) {
			...
			$connected_ids = OsConnectorHelper::get_connected_object_ids(
				'location_id',
				[
					'agent_id'   => OsStepsHelper::$booking_object->agent_id,
					'service_id' => OsStepsHelper::$booking_object->service_id,
				] 
			);
			...
			if ( ! empty( $show_locations_arr ) ) {
				$locations_model->where_in( 'id', $show_locations_arr );
			}
			...
			$locations_controller->format_render(
				'_step_booking__locations',
				[],
				[
					'step_code'        => $step_code,
	...
	public static function add_booking_location_to_webhook( $vars, $booking ) {
		if ( $booking->location_id ) {
			$vars['location'] = $booking->location->name;
		}
		return $vars;
	}
	...
	/**
	 * Adds location as a queryable property of a booking, so it can be picked as
	 * a trigger condition in workflows. Bound to latepoint_model_properties_to_query,
	 * which is what the condition form reads - keys are bare attribute names, the
	 * object prefix is added by ProcessEvent::get_properties_for_object_code().
	 */
	public static function add_location_to_properties_to_query( $properties, $model ) {
		if ( is_a( $model, 'OsBookingModel' ) ) {
			$properties['location_id'] = __( 'Location', 'latepoint-pro-features' );
		}
		return $properties;
	}
	...
	/**
	 * Supplies the list of locations for the value selector of a workflow trigger
	 * condition, for both the current and the previous booking.
	 */
	public static function add_location_values_for_process_event_trigger_condition( $values, $property, $property_object = '', $property_attribute = '' ) {
		if ( $property_attribute == 'location_id' && in_array( $property_object, [ 'booking', 'old_booking' ] ) ) {
