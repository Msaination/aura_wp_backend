...
class OsFeatureWhiteLabelHelper {
	...
	/**
	 * Remove the "View details" link, and replace any remaining "LatePoint" text in plugin row
	 * meta items (covers "Required by / Requires" dependency notices added by WP core).
	 *
	...
	public static function filter_plugin_row_meta( array $plugin_meta, string $plugin_file ): array {
		...
		// The modal this link opens is rendered from data fetched off the remote plugin API, so it
		// shows the original branding whatever is set here, and its URL carries the real plugin slug.
		$plugin_meta = array_values(
			array_filter(
				$plugin_meta,
				static function ( $item ) {
					return strpos( $item, 'open-plugin-details-modal' ) === false;
				}
			)
		);
