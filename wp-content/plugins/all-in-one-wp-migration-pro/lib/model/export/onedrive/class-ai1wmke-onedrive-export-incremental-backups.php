...
class Ai1wmke_OneDrive_Export_Incremental_Backups {
	...
	public static function execute( $params, $onedrive = null ) {
		...
		if ( ( $incremental_list = ai1wm_open( ai1wm_incremental_backups_list_path( $params ), 'wb' ) ) ) {
			try {
				if ( ( $response = $onedrive->list_folder( $params['incremental_folder_id'], "name eq 'incremental.backups.list'" ) ) ) {
		...
		if ( ( $incremental_list = ai1wm_open( ai1wm_incremental_backups_list_path( $params ), 'rb' ) ) ) {
			while ( ( $row = ai1wm_getcsv( $incremental_list ) ) !== false ) {
				list( $file_index, $file_id, $file_path, $file_size, $file_mtime ) = $row;
				$incremental_files[ $file_index ] = array( $file_id, $file_path, $file_size, $file_mtime ); // phpcs:ignore Generic.Formatting.MultipleStatementAlignment.NotSameWarning
			}
		...
		if ( ( $incremental_content_list = ai1wm_open( ai1wm_incremental_content_list_path( $params ), 'rb' ) ) ) {
			try {
				$onedrive->upload_file( $incremental_content_list, 'incremental.content.list', filesize( ai1wm_incremental_content_list_path( $params ) ), $params['incremental_folder_id'] );
			} catch ( Ai1wmke_Error_Exception $e ) {
				throw $e;
			}
		...
		if ( ( $incremental_media_list = ai1wm_open( ai1wm_incremental_media_list_path( $params ), 'rb' ) ) ) {
			try {
				$onedrive->upload_file( $incremental_media_list, 'incremental.media.list', filesize( ai1wm_incremental_media_list_path( $params ) ), $params['incremental_folder_id'] );
			} catch ( Ai1wmke_Error_Exception $e ) {
				throw $e;
			}
		...
		if ( ( $incremental_plugins_list = ai1wm_open( ai1wm_incremental_plugins_list_path( $params ), 'rb' ) ) ) {
			try {
				$onedrive->upload_file( $incremental_plugins_list, 'incremental.plugins.list', filesize( ai1wm_incremental_plugins_list_path( $params ) ), $params['incremental_folder_id'] );
			} catch ( Ai1wmke_Error_Exception $e ) {
				throw $e;
			}
		...
		if ( ( $incremental_themes_list = ai1wm_open( ai1wm_incremental_themes_list_path( $params ), 'rb' ) ) ) {
			try {
				$onedrive->upload_file( $incremental_themes_list, 'incremental.themes.list', filesize( ai1wm_incremental_themes_list_path( $params ) ), $params['incremental_folder_id'] );
			} catch ( Ai1wmke_Error_Exception $e ) {
				throw $e;
			}
		...
		if ( ( $incremental_backups_list = ai1wm_open( ai1wm_incremental_backups_list_path( $params ), 'rb' ) ) ) {
			try {
				$onedrive->upload_file( $incremental_backups_list, 'incremental.backups.list', filesize( ai1wm_incremental_backups_list_path( $params ) ), $params['incremental_folder_id'] );
			} catch ( Ai1wmke_Error_Exception $e ) {
				throw $e;
			}
