...
class Ai1wmke_Dropbox_Export_Incremental_Media {
	...
	public static function execute( $params, $dropbox = null ) {
		...
		if ( ( $incremental_list = ai1wm_open( ai1wm_incremental_media_list_path( $params ), 'wb' ) ) ) {
			try {
				$dropbox->get_file( $incremental_list, sprintf( '/%s/incremental-backups/incremental.media.list', $params['folder_path'] ) );
