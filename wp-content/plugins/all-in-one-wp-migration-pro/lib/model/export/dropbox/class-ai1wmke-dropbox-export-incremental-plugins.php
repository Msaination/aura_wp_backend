...
class Ai1wmke_Dropbox_Export_Incremental_Plugins {
	...
	public static function execute( $params, $dropbox = null ) {
		...
		if ( ( $incremental_list = ai1wm_open( ai1wm_incremental_plugins_list_path( $params ), 'wb' ) ) ) {
			try {
				$dropbox->get_file( $incremental_list, sprintf( '/%s/incremental-backups/incremental.plugins.list', $params['folder_path'] ) );
