...
class Ai1wmke_Dropbox_Export_Incremental_Content {
	...
	public static function execute( $params, $dropbox = null ) {
		...
		if ( ( $incremental_list = ai1wm_open( ai1wm_incremental_content_list_path( $params ), 'wb' ) ) ) {
			try {
				$dropbox->get_file( $incremental_list, sprintf( '/%s/incremental-backups/incremental.content.list', $params['folder_path'] ) );
