...
class Ai1wmke_B2_Export_Incremental_Content {
	...
	public static function execute( $params, $b2 = null ) {
		...
		if ( ( $incremental_list = ai1wm_open( ai1wm_incremental_content_list_path( $params ), 'wb' ) ) ) {
			try {
				$b2->download_file_by_name( $incremental_list, sprintf( '/file/%s/%s/incremental-backups/incremental.content.list', $params['bucket_name'], $params['folder_name'] ) );
