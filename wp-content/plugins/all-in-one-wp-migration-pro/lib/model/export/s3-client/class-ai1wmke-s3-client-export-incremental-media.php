...
class Ai1wmke_S3_Client_Export_Incremental_Media {
	...
	public static function execute( $params, $s3 = null ) {
		...
		if ( ( $incremental_list = ai1wm_open( ai1wm_incremental_media_list_path( $params ), 'wb' ) ) ) {
			try {
				$s3->get_file( $incremental_list, sprintf( '/%s/incremental-backups/incremental.media.list', $params['folder_name'] ), $params['bucket_name'], $params['region_name'] );
