...
class Ai1wmke_DigitalOcean_Import_Incremental_Download {
	...
	public static function execute( $params, $digitalocean = null ) {
		...
		if ( ! isset( $params['file_path'] ) ) {
			throw new Ai1wm_Import_Exception( __( 'DigitalOcean Spaces File Path is not specified.', AI1WMKE_PLUGIN_NAME ) );
		}

		// Validate file size
		if ( ! isset( $params['file_size'] ) ) {
			throw new Ai1wm_Import_Exception( __( 'DigitalOcean Spaces File Size is not specified.', AI1WMKE_PLUGIN_NAME ) );
		}

		// Validate bucket name
		if ( ! isset( $params['bucket_name'] ) ) {
			throw new Ai1wm_Import_Exception( __( 'DigitalOcean Spaces Bucket Name is not specified.', AI1WMKE_PLUGIN_NAME ) );
		}

		// Validate region name
		if ( ! isset( $params['region_name'] ) ) {
			throw new Ai1wm_Import_Exception( __( 'DigitalOcean Spaces Region Name is not specified.', AI1WMKE_PLUGIN_NAME ) );
		}

		// Validate folder path
		if ( ! isset( $params['folder_path'] ) ) {
			throw new Ai1wm_Import_Exception( __( 'DigitalOcean Spaces Folder Path is not specified.', AI1WMKE_PLUGIN_NAME ) );
		}
		...
		if ( ( $archive = fopen( ai1wm_archive_path( $params ), 'cb' ) ) ) {
			if ( ( fseek( $archive, $params['archive_offset'] ) !== -1 ) ) {
				try {
					...
					$digitalocean->get_file( $archive, sprintf( '/%s/%s', $params['folder_path'], $params['file_path'] ), $params['bucket_name'], $params['region_name'], $params['file_range_start'], $params['file_range_end'] );
