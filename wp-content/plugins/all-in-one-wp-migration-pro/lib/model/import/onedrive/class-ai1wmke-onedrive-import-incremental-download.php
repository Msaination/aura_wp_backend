...
class Ai1wmke_OneDrive_Import_Incremental_Download {
	...
	public static function execute( $params, $onedrive = null ) {
		...
		if ( ! isset( $params['file_index'] ) ) {
			throw new Ai1wm_Import_Exception( __( 'OneDrive File Index is not specified.', AI1WMKE_PLUGIN_NAME ) );
		}

		// Validate file path
		if ( ! isset( $params['file_id'] ) ) {
			throw new Ai1wm_Import_Exception( __( 'OneDrive File ID is not specified.', AI1WMKE_PLUGIN_NAME ) );
		}

		// Validate file size
		if ( ! isset( $params['file_size'] ) ) {
			throw new Ai1wm_Import_Exception( __( 'OneDrive File Size is not specified.', AI1WMKE_PLUGIN_NAME ) );
		}
