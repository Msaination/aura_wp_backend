...
class Ai1wmke_Dropbox_Import_Incremental_Download {
	...
	public static function execute( $params, $dropbox = null ) {
		...
		if ( ! isset( $params['file_index'] ) ) {
			throw new Ai1wm_Import_Exception( __( 'Dropbox File Index is not specified.', AI1WMKE_PLUGIN_NAME ) );
		}

		// Validate file path
		if ( ! isset( $params['file_path'] ) ) {
			throw new Ai1wm_Import_Exception( __( 'Dropbox File Path is not specified.', AI1WMKE_PLUGIN_NAME ) );
		}

		// Validate file size
		if ( ! isset( $params['file_size'] ) ) {
			throw new Ai1wm_Import_Exception( __( 'Dropbox File Size is not specified.', AI1WMKE_PLUGIN_NAME ) );
		}
