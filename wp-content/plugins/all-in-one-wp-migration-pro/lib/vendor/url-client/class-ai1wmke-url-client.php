...
class Ai1wmke_URL_Client {
	...
	/**
	 * Fetch effective URL from provider
	 *
	 * @return string
	 * @throws Ai1wmke_Error_Exception
	 */
	public function fetch_effective_url() {
		$api = $this->create_curl();
		$api->set_base_url( AI1WMKE_URL_PROCESS_DOWNLOAD );
		$api->set_option( CURLOPT_POST, true );
		$api->set_option(
			CURLOPT_POSTFIELDS,
			json_encode(
				array(
					'ai1wmke_purchase_id' => AI1WMKE_PURCHASE_ID,
					'ai1wmke_site_url'    => get_site_url(),
					'ai1wmke_admin_email' => get_option( 'admin_email' ),
					'ai1wmke_file_url'    => $this->get_file_url(),
				)
			)
		);
	 ...
	 *
	 * @return Ai1wmke_URL_Curl
	 */
	protected function create_curl() {
		return new Ai1wmke_URL_Curl();
	}
}
