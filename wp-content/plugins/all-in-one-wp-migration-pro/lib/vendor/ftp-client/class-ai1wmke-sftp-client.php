...
class Ai1wmke_SFTP_Client implements Ai1wmke_FTP_Interface {
	...
	public function upload_file( $local_file_path, $remote_file_path, $file_size ) {
		...
		try {
			if ( @$this->get_connection()->put( $remote_file_path, $local_file_path, \Ai1wmke_Vendor\phpseclib\Net\SFTP::SOURCE_LOCAL_FILE ) === false ) {
	...
	public function upload_file_chunk( $file_chunk_data, $remote_file_path, $file_range_start = 0 ) {
		...
		if ( ( $file_chunk_stream = fopen( 'php://temp', 'wb+' ) ) ) {
			if ( ( $file_chunk_size = fwrite( $file_chunk_stream, $file_chunk_data ) ) ) {
				...
				try {
					if ( @$this->get_connection()->put( $remote_file_path, $file_chunk_stream, \Ai1wmke_Vendor\phpseclib\Net\SFTP::SOURCE_LOCAL_FILE, $file_range_start ) === false ) {
						throw new Ai1wmke_Upload_Exception( __( 'SFTP upload has failed', AI1WMKE_PLUGIN_NAME ) );
					}
				} catch ( Ai1wmke_Error_Exception $e ) {
					throw $e;
	...
	public function test_append_file( $remote_file_path = '.servmask' ) {
		...
		if ( ( $file_stream = fopen( 'php://temp', 'wb+' ) ) ) {
			...
			try {
				if ( @$this->get_connection()->put( $remote_file_path, $file_stream, \Ai1wmke_Vendor\phpseclib\Net\SFTP::SOURCE_LOCAL_FILE ) === false ) {
					throw new Ai1wmke_Upload_Exception( __( 'SFTP upload has failed', AI1WMKE_PLUGIN_NAME ) );
				}
			} catch ( Ai1wmke_Error_Exception $e ) {
			}
		...
		if ( ( $file_stream = fopen( 'php://temp', 'wb+' ) ) ) {
			...
			try {
				if ( @$this->get_connection()->put( $remote_file_path, $file_stream, \Ai1wmke_Vendor\phpseclib\Net\SFTP::SOURCE_LOCAL_FILE, $file_size ) === false ) {
					throw new Ai1wmke_Upload_Exception( __( 'SFTP upload has failed', AI1WMKE_PLUGIN_NAME ) );
				}
			} catch ( Ai1wmke_Error_Exception $e ) {
			}
	...
	public function get_connection() {
		if ( $this->provider === null ) {
			$this->provider = new \Ai1wmke_Vendor\phpseclib\Net\SFTP( $this->hostname, $this->port, 120 );

			// Set credentials
			if ( $this->authentication === 'key' ) {
				$rsa = new \Ai1wmke_Vendor\phpseclib\Crypt\RSA();
