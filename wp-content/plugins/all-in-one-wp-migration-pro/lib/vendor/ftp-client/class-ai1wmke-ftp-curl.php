...
class Ai1wmke_FTP_Curl implements Ai1wmke_FTP_Interface {
	 ...
	 * @var string
	 */
	protected $remote_file_path = null;
	...
	public function __construct( $hostname, $username, $password, $directory = '/', $port = 21, $active = false, $ssl = false ) {
		if ( ! extension_loaded( 'curl' ) ) {
			throw new Ai1wmke_Error_Exception( __( 'FTP Extension requires PHP cURL extension. <a href="https://help.servmask.com/knowledgebase/curl-missing-in-php-installation/" target="_blank">Technical details</a>', AI1WMKE_PLUGIN_NAME ) );
		}
	...
	public function create_folder( $folder_path ) {
		...
		try {
			$this->make_request( $folder_path, array( CURLOPT_FTP_CREATE_MISSING_DIRS => true ) );
			...
			if ( ( $meta = preg_split( '/\s+/', trim( $item ) ) ) && ( $name = array_pop( $meta ) ) ) {
				if ( ( $name = trim( $name ) ) && strpos( $name, '.' ) !== 0 ) {
					...
					if ( preg_match( '/(.+?)-(\d+?)-(\d+?)-(.+?)\.wpress/', $name, $matches ) ) {
						$objects[] = array(
							'name'  => $name,
							'path'  => $this->sanitize_path( $this->left_trim_forward_slash( sprintf( '/%s/%s', $folder_path, $name ) ) ),
	...
	public function raw_list_folder( $folder_path ) {
		...
		try {
			$items = $this->make_request( $folder_path, array( CURLOPT_CUSTOMREQUEST => 'LIST -tlA' ) );
		} catch ( Ai1wmke_Error_Exception $e ) {
			throw $e;
		}
	...
	public function get_file_content( $remote_file_path ) {
		...
		try {
			...
			return $this->make_request( $remote_file_path, $options );
		} catch ( Ai1wmke_Error_Exception $e ) {
			throw $e;
		}
	}
	...
	public function remove_file( $file_path ) {
		...
		$folder_path = $this->sanitize_path( sprintf( '/%s/', $this->directory ) );

		try {
			$this->make_request( $folder_path, array( CURLOPT_QUOTE => array( sprintf( 'DELE /%s', $this->left_trim_forward_slash( $file_path ) ) ) ) );
		} catch ( Ai1wmke_Error_Exception $e ) {
			return false;
		}
	...
	public function test_connection() {
		...
		try {
			$this->make_request( $folder_path, array( CURLOPT_TIMEOUT => 120 ) );
		} catch ( Ai1wmke_Error_Exception $e ) {
			throw $e;
		}
	...
	public function test_append_file( $remote_file_path = '.servmask' ) {
		...
		try {
			if ( ( $remote_file_data = $this->make_request( $remote_file_path ) ) ) {
	...
	protected function make_request( $path, $options = array() ) {
		...
		// Set FTP URL
		curl_setopt( $this->handler, CURLOPT_URL, sprintf( 'ftp://%s:%d/%s', $this->hostname, $this->port, $this->left_trim_forward_slash( $path ) ) );
		...
		if ( $response === false ) {
			if ( ( $errno = curl_errno( $this->handler ) ) ) {
				switch ( $errno ) {
					...
					case 7:
						if ( $this->ssl ) {
							throw new Ai1wmke_Error_Exception( __( 'Unable to connect to FTPS server. Please check your FTPS hostname, port, and active mode settings. <a href="https://help.servmask.com/knowledgebase/ftp-error-codes/#invalid-hostname" target="_blank">Technical details</a>', AI1WMKE_PLUGIN_NAME ) );
						} else {
							throw new Ai1wmke_Error_Exception( __( 'Unable to connect to FTP server. Please check your FTP hostname, port, and active mode settings. <a href="https://help.servmask.com/knowledgebase/ftp-error-codes/#invalid-hostname" target="_blank">Technical details</a>', AI1WMKE_PLUGIN_NAME ) );
						}

					case 9:
						if ( $this->ssl ) {
							throw new Ai1wmke_Error_Exception( __( 'Unable to change FTPS directory. Please ensure that you have permission on the server. <a href="https://help.servmask.com/knowledgebase/ftp-error-codes/#invalid-directory" target="_blank">Technical details</a>', AI1WMKE_PLUGIN_NAME ) );
						} else {
							throw new Ai1wmke_Error_Exception( __( 'Unable to change FTP directory. Please ensure that you have permission on the server. <a href="https://help.servmask.com/knowledgebase/ftp-error-codes/#invalid-directory" target="_blank">Technical details</a>', AI1WMKE_PLUGIN_NAME ) );
						}

					case 23:
						if ( $this->ssl ) {
							throw new Ai1wmke_Write_Error_Exception( __( 'Unable to download file from FTPS server. Please ensure that you have enough disk space. <a href="https://help.servmask.com/knowledgebase/ftp-error-codes/#write-error" target="_blank">Technical details</a>', AI1WMKE_PLUGIN_NAME ) );
						} else {
							throw new Ai1wmke_Write_Error_Exception( __( 'Unable to download file from FTP server. Please ensure that you have enough disk space. <a href="https://help.servmask.com/knowledgebase/ftp-error-codes/#write-error" target="_blank">Technical details</a>', AI1WMKE_PLUGIN_NAME ) );
						}

					case 28:
						if ( $this->ssl ) {
							throw new Ai1wmke_Operation_Timedout_Exception( __( 'Connecting to FTPS server timed out. Please check FTPS hostname, port, username, password, and active mode settings. <a href="https://help.servmask.com/knowledgebase/ftp-error-codes/#operation-timedout" target="_blank">Technical details</a>', AI1WMKE_PLUGIN_NAME ) );
						} else {
							throw new Ai1wmke_Operation_Timedout_Exception( __( 'Connecting to FTP server timed out. Please check FTP hostname, port, username, password, and active mode settings. <a href="https://help.servmask.com/knowledgebase/ftp-error-codes/#operation-timedout" target="_blank">Technical details</a>', AI1WMKE_PLUGIN_NAME ) );
						}

					case 67:
						if ( $this->ssl ) {
							throw new Ai1wmke_Error_Exception( __( 'Unable to login to FTPS server. Please check your username and password. <a href="https://help.servmask.com/knowledgebase/ftp-error-codes/#invalid-credentials" target="_blank">Technical details</a>', AI1WMKE_PLUGIN_NAME ) );
						} else {
							throw new Ai1wmke_Error_Exception( __( 'Unable to login to FTP server. Please check your username and password. <a href="https://help.servmask.com/knowledgebase/ftp-error-codes/#invalid-credentials" target="_blank">Technical details</a>', AI1WMKE_PLUGIN_NAME ) );
						}

					default:
						if ( $this->ssl ) {
							throw new Ai1wmke_Connect_Exception( sprintf( __( 'Unable to connect to FTPS. Error code: %s. <a href="https://help.servmask.com/knowledgebase/ftp-error-codes/#%s" target="_blank">Technical details</a>', AI1WMKE_PLUGIN_NAME ), $errno, $errno ) );
						} else {
							throw new Ai1wmke_Connect_Exception( sprintf( __( 'Unable to connect to FTP. Error code: %s. <a href="https://help.servmask.com/knowledgebase/ftp-error-codes/#%s" target="_blank">Technical details</a>', AI1WMKE_PLUGIN_NAME ), $errno, $errno ) );
						}
				}
			}
		}
		...
		if ( $http_code === 429 ) {
			throw new Ai1wmke_Rate_Limit_Exception( sprintf( __( 'Too many requests. Please try again later. Error code: %s <a href="https://help.servmask.com/knowledgebase/ftp-error-codes/#%s" target="_blank">Technical details</a>', AI1WMKE_PLUGIN_NAME ), $http_code, $http_code ) );
		}

		if ( $http_code >= 500 ) {
			throw new Ai1wmke_Internal_Server_Error_Exception( sprintf( __( 'Internal Server Error. Please try again later. Error code: %s <a href="https://help.servmask.com/knowledgebase/ftp-error-codes/#%s" target="_blank">Technical details</a>', AI1WMKE_PLUGIN_NAME ), $http_code, $http_code ) );
		}

		if ( $http_code >= 400 ) {
			if ( isset( $this->messages[ $http_code ] ) ) {
				throw new Ai1wmke_Error_Exception( sprintf( __( '%s. <a href="https://help.servmask.com/knowledgebase/ftp-error-codes/#%s" target="_blank">Technical details</a>', AI1WMKE_PLUGIN_NAME ), $this->messages[ $http_code ], $http_code ) );
			} else {
				throw new Ai1wmke_Error_Exception( sprintf( __( 'Error code: %s. <a href="https://help.servmask.com/knowledgebase/ftp-error-codes/#%s" target="_blank">Technical details</a>', AI1WMKE_PLUGIN_NAME ), $http_code, $http_code ) );
			}
		}
