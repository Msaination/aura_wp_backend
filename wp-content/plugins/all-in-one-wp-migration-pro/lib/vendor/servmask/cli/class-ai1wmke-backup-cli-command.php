...
class Ai1wmke_Backup_CLI_Command extends Ai1wmke_Backup_CLI_Base {
	...
	public function browse_backup( $args = array(), $assoc_args = array() ) {
		...
		try {
			...
			if ( ! $archive->is_valid() ) {
				WP_CLI::error( __( 'The archive file appears to be corrupted. Follow https://help.servmask.com/knowledgebase/corrupted-archive/ this article for possible fixes.', AI1WMKE_PLUGIN_NAME ) );
				exit;
			}
	...
	public function reset( $args = array(), $assoc_args = array() ) {
		...
		try {
			Ai1wmke_Reset_Tools_Controller::reset_tools( $params );
		} catch ( Exception $e ) {
