	...
	protected function build_export_params( $args = array(), $assoc_args = array() ) {
		...
		if ( isset( $assoc_args['compression'] ) ) {
			...
			if ( ! ai1wm_has_compression_type( $assoc_args['compression'] ) ) {
				WP_CLI::error( sprintf( __( 'Your system doesn\'t support %s compression.', AI1WMKE_PLUGIN_NAME ), $assoc_args['compression'] ) );
				exit;
			}
					...
					switch ( $file ) {
						...
						default:
							if ( ! file_exists( WP_CONTENT_DIR . DIRECTORY_SEPARATOR . untrailingslashit( $file ) ) ) {
								WP_CLI::warning( __( 'Unknown file or folder: ', AI1WMKE_PLUGIN_NAME ) . $file );
								break;
							}
							$excluded_files[] = $file;
					}
		...
		if ( isset( $assoc_args['exclude-database'] ) ) {
			...
			if ( isset( $assoc_args['exclude-tables'] ) ) {
				...
				if ( $assoc_args['exclude-tables'] === true || empty( $assoc_args['exclude-tables'] ) ) {
					...
					$excluded_tables = array_intersect( $all_tables, array_filter( array_map( 'trim', explode( ',', $assoc_args['exclude-tables'] ) ) ) );
				}
			...
			if ( isset( $assoc_args['include-tables'] ) ) {
				...
				if ( $assoc_args['include-tables'] === true || empty( $assoc_args['include-tables'] ) ) {
					...
					$included_tables = array_intersect( $all_tables, array_filter( array_map( 'trim', explode( ',', $assoc_args['include-tables'] ) ) ) );
				}
