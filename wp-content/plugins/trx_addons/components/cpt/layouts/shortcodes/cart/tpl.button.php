...
if ( ! empty( $show_cart ) ) {
	...
	// Hide the product count badge when the cart is empty (if the corresponding option is enabled)
	$sc_layouts_cart_items_short_class = 'sc_layouts_cart_items_short';
	if ( $allow_sc_styles_in_elementor && ! empty( $args['hide_empty_badge'] ) ) {
		$sc_layouts_cart_items_short_class .= ' sc_layouts_cart_hide_empty';
		if ( (int) $cart_items === 0 ) {
			$sc_layouts_cart_items_short_class .= ' sc_layouts_cart_items_short_empty';
		}
	}
	...
	?>><?php
		...
		<span class="sc_layouts_item_details sc_layouts_cart_details<?php
			if ( $allow_sc_styles_in_elementor && ! empty( $args['hide_empty_badge'] ) ) {
				echo ' ' . esc_attr( 'sc_layouts_item_details_hide_empty_badge' );
			}
			if ( empty( $args['text'] ) ) {
				echo ' ' . esc_attr( 'sc_layouts_item_details_without_line1' );
			}
			?>"<?php
			echo wp_kses( $button_link_data, 'trx_addons_kses_content' );
		?>>
		...
		<span class="<?php echo esc_attr( $sc_layouts_cart_items_short_class ); ?>"><?php echo esc_html( $cart_items ); ?></span><?php
							...
							sprintf( '<div id="%1$s" class="sc_layouts_cart_button_wrap'
									...
									. '">'
										...
										. ( ! empty( $cart_url ) ? '<a href="' . esc_url( $cart_url ) . '"' : '<span' ) . ' class="sc_layouts_cart_button">'
											...
											. '<span class="' . esc_attr( $sc_layouts_cart_items_short_class ) . '">%2$d</span>'
