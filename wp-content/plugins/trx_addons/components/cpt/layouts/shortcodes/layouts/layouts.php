	...
	function trx_addons_sc_layouts($atts, $content = '') {	
		...
		if ( $atts['type'] == 'panel' ) {
			...
			foreach ( $breakpoints as $bp => $bpv ) {
				...
				if ( ! isset( $atts['size' . $suffix] ) && $bp != 'desktop' ) {
					continue;
				}
				if ( empty( $atts['size' . $suffix] ) ) {
					$atts['size' . $suffix] = $bp == 'desktop' ? 'auto' : '';
				} else {
					$atts['size' . $suffix] = trx_addons_prepare_css_value( $atts['size' . $suffix] );
				}
				if ( empty( $atts['size' . $suffix] ) ) {
					continue;
				}
				$css_styles = trx_addons_get_css_dimensions_from_values(
								in_array($atts['position'], array('left', 'right')) ? $atts['size' . $suffix] : '',
								in_array($atts['position'], array('top', 'bottom')) ? $atts['size' . $suffix] : ''
				);
				if ( $bp == 'desktop' ) {
					$panel_class = trx_addons_add_inline_css_class( $css_styles );
					$atts['class'] .= ( ! empty( $atts['class'] ) ? ' ' : '') . $panel_class;
					$atts['panel_class'] = $panel_class;
				} else {
					trx_addons_add_inline_css(
						"@media (max-width: {$bpv}px) {
							.{$panel_class} {
								" . $css_styles . "
							}
						}"
