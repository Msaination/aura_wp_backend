	...
	function trx_addons_is_learnpress_page() {
		...
		if ( trx_addons_exists_learnpress() && ! is_search() ) {
			$rez = is_learnpress()
					...
					|| trx_addons_check_url( '/course-builder/' )
...
if ( ! function_exists( 'trx_addons_learnpress_disable_tinymce_plugins' ) ) {
	add_filter( 'trx_addons_filter_editor_extensions_enable', 'trx_addons_learnpress_disable_tinymce_plugins' );
	/**
	 * Disable Tiny MCE plugins on some LearnPress pages
	 * 
	* @param boolean $enable      true - plugins enabled
	*/
	function trx_addons_learnpress_disable_tinymce_plugins( $enable ) {
		if ( trx_addons_exists_learnpress() && trx_addons_is_learnpress_page() ) {
			$enable = false;
		}
		return $enable;
	}
