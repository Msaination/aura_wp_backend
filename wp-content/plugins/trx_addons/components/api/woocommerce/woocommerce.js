			...
			cont.find( '.variations_form.cart:not(.inited)' ).each( function() {
				...
				trx_addons_attribs.on( 'click', function(e) {
					...
					if ( ! $attrib.hasClass('trx_addons_attrib_disabled') ) {
						...
						// Way #2: still not compatible with non-latin attribute names, because the 'data-attrib' (urlencode)
						// and the select's id (sanitize_title) encode non-ASCII chars in a different hex case (%D1 vs %d1),
						// so the select is not found, its 'change' is not fired and the variations are not rechecked.
						// $attrib.parents('.trx_addons_attrib_extended').parent().find('[id="'+attrib_name+'"]').val(term).trigger('change');
						
						// New way: find the select by the DOM relation (it is a neighbour of our extended block in the same cell),
						// so it does not depend on the attribute name encoding at all.
						$attrib.parents('.trx_addons_attrib_extended').parent().find('select').val(term).trigger('change');
					}
					return false;
				} );
			} );
			...
			if ( search_forms.length ) {
				search_forms.each( function() {
					...
					form
						...
						// Show/hide items with specified keywords ('filter' style)
						.on('keyup', '.sc_form_field_items_filter_text', function(e) {
							if ( type == 'filter' ) {
								var $self = jQuery(this),
									value = $self.val().toLowerCase(),
									wrap  = $self.parents('.sc_form_field_wrap');
								wrap.find('.sc_form_field_item').each( function() {
									var $item = jQuery(this);
									$item.toggleClass('trx_addons_hidden_important', value !== '' && ('' + $item.data('value')).toLowerCase().indexOf(value) === -1);
								} );
							}
						})
					...
					if ( type == 'filter' && ! reopened ) {
						...
						if ( last_opened_filter ) {
							...
							} else if ( trx_addons_window_scroll_top() > 0 ) {
					...
					function trx_addons_woocommerce_search_form_reload_products( form, filter, selector, force_reload_page ) {
						...
						if ( params !== false ) {
							...
							// Add scroll top position to the query (commented to prevent scroll to blank window area if sidebar with filters too long)
				...
				jQuery.get( shop_url ).done( function( response ) {
					...
					function list_products_replace( remove_loading ) {
						...
						if ( widget_number ) {
							...
							if ( fields_wrap.length ) {
								...
								if ( last_opened_filter ) {// && ! fields_wrap.parents('.trx_addons_woocommerce_search ').hasClass('trx_addons_woocommerce_search_apply') ) {
									...
									if ( last_clicked_item ) {
										field.find('.sc_form_field_item[data-value="'+last_clicked_item+'"]').get(0).focus();
									}
								}
						...
						if ( trx_addons_window_scroll_top() > 0 ) {
							trx_addons_document_animate_to( list_products_wrap );
						}
						reload_is_busy = false;
					}
				} );
