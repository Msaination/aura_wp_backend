	...
	function trx_addons_woocommerce_attrib_init() {
		...
		if ( ! empty( $attribute_taxonomies ) ) {
			foreach ( $attribute_taxonomies as $attribute ) {
				...
				add_filter( 'manage_' . $tax . '_custom_column', 'trx_addons_woocommerce_attrib_fill_custom_column',  9, 3 );
			}
		}
	}
}
...
if ( ! function_exists( 'trx_addons_woocommerce_attrib_fill_custom_column' ) ) {
	//Hook of the add_filter('manage_pa_xxx_custom_column',	'trx_addons_woocommerce_attrib_fill_custom_column', 9, 3);
	/**
...
if ( ! function_exists( 'trx_addons_woocommerce_attrib_show_single_product' ) ) {
	...
	function trx_addons_woocommerce_attrib_show_single_product( $html, $args ) {
		...
		if ( in_array( $type, array( 'image', 'color', 'button' ) ) ) {
			...
			if ( $output ) {
				// Way 1: Add attribute class anyway (may conflict with 3rd-party plugins)
				// $html = str_replace( '<select ', '<select class="trx_addons_attrib_' . esc_attr( $type ) . '" style="display:none !important;" ', $html );

				// Way 2: Add our class to the <select> opening tag. If a class attribute already exists
				// (e.g. added by another plugin), append our class to it instead of creating a duplicate one.
				$html = preg_replace_callback(
					...
					function ( $matches ) use ( $type ) {
						...
						if ( preg_match( '/\sclass\s*=\s*(["\'])(.*?)\1/i', $attrs ) ) {
							...
							$attrs = preg_replace(
								'/(\sclass\s*=\s*(["\']))(.*?)(\2)/i',
								'$1$3 trx_addons_attrib_' . esc_attr( $type ) . '$4',
								$attrs,
								1
							); 
						} else {
							// No class attribute present - add a new one.
							$attrs = ' class="trx_addons_attrib_' . esc_attr( $type ) . '"' . $attrs;
						}
						return '<select' . $attrs . ' style="display:none !important;">';
					},
					$html,
					1
				);
				...
				$html .= '<div id="' . esc_attr( ! empty( $args['id'] ) ? $args['id'] : sanitize_title( $args['attribute'] ) ) . '_attrib_extended"'
							...
							. ' data-attrib="' . esc_attr( strtolower( urlencode( $args['attribute'] ) ) ) . '">'
							. $output
						. '</div>';
			}
		}
