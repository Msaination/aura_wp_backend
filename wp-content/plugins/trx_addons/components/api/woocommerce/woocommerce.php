...
if ( ! function_exists( 'trx_addons_woocommerce_get_list_terms_fix_attributes' ) ) {
	add_filter('trx_addons_filter_get_list_terms_args', 'trx_addons_woocommerce_get_list_terms_fix_attributes', 1, 3 );
	/**
	 * Fix args for get_terms for the sites with many attribute taxonomies
	 * 
	 * @param array  $args      Arguments for get_terms
	 * @param string $taxonomy  Taxonomy for get_terms
	 * @param array  $opt       Additional options
	 *
	 * @hooked trx_addons_filter_get_list_terms_args, 1
	 * 
	 * @return array            Modified arguments
	 */
	function trx_addons_woocommerce_get_list_terms_fix_attributes( $args, $taxonomy, $opt ) {
		foreach( (array)$taxonomy as $t ) {
			if ( substr( $t, 0, 3 ) == 'pa_' ) {	//taxonomy_is_product_attribute( $t ) ) {
				// Turn parameters to make query faster
				$args['hierarchical'] = false;	// If products on the site have many attributes - 'hierarchical' == true make query very slow
				$args['hide_empty'] = 1;		// If products on the site have many attributes - 'hide_empty' == false make query very slow
			}
		}
		return $args;
	}
...
if ( ! function_exists( 'trx_addons_woocommerce_get_attribute_by_id' ) ) {
	...
	function trx_addons_woocommerce_get_attribute_by_id( $id, $field = '' ) {
		...
		return ! empty( $field )
					? ( ! empty( $attributes[ $id ]->{$field} ) ? $attributes[ $id ]->{$field} : '' )
					: $attributes[ $id ];
	}
}
	...
	function trx_addons_woocommerce_load_scripts_front( $force = false ) {
		...
		trx_addons_enqueue_optimized( 'woocommerce', $force, array(
			...
			'check' => array(
				...
				// Widgets from the plugin "Trx WooCommerce Extension"
				array( 'type' => 'elm', 'sc' => '"widgetType":"trx-wcext-' ),
			)
		) );
	}
}
