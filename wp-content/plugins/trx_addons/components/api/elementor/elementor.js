...
*/
	...
	function trx_addons_elementor_animate_item( item, idx, total, force, need_animation ) {
		...
		if ( ! need_animation ) {
			...
			setTimeout( function() {
				if ( item.hasClass( 'wait-for-animation' ) ) {
					item.removeClass('wait-for-animation');
				}
				if ( item.hasClass('elementor-invisible') ) {
					item.removeClass('elementor-invisible');
				}
				...
				if ( ! item.hasClass( 'animated' ) ) {
					item.addClass('animated' + (item_duration ? ' animated-' + item_duration : '')).addClass(item_animation);
				}
			}, item_delay );
		}
		// Check if the item inside the slider - remove invisible classes after timeout to prevent hide the duplicate slides after animation
		if ( item.hasClass( 'swiper-slide' ) || item.parent().hasClass( 'swiper-slide' ) ) {
			var slider = item.closest( '.swiper-container' ),
				slides_visible = slider.find( '.swiper-slide:not(.swiper-slide-duplicate)' ),
				wait_for = trx_addons_elementor_animate_items_delay( item_params, sc, item, idx + 1, total ) - item_delay;
			if ( slider.length && ! slider.data( 'invisible-remove-added' ) ) {
				slider.data( 'invisible-remove-added', true );
				setTimeout( function() {
					slider.find( '.trx_addons_invisible' )
						.removeClass( 'elementor-invisible trx_addons_invisible' );
					slider.siblings( '.trx_addons_invisible' ).each( function() {
						var control = jQuery( this );
						if ( control.hasClass( 'trx_addons_invisible' ) ) {
							control
				...
				}, slides_visible.length * wait_for + 100 );
			}
		}
	}
