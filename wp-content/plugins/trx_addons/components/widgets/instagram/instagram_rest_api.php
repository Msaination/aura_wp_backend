		...
		if ( substr($output, 0, 1) == '{' ) {
			...
			if ( ! empty( $output['data'] ) && is_array( $output['data'] ) ) {
				...
				foreach ( $output['data'] as $image ) {
					...
					$media = array(
									'url'    => esc_url_raw( $image['media_url'] ),
									'width'  => '',
									'height' => '',
									);
					...
					if ( $type == 'video' && ! empty( $image['thumbnail_url'] ) ) {
						$thumb = array(
										'url'    => esc_url_raw( $image['thumbnail_url'] ),
										'width'  => '',
										'height' => '',
										);
						$thumbs = array(
					...
					$instagram[] = array_merge(
						array(
							'type'		=> $type,
							'link'		=> ! empty( $image['username'] ) ? esc_url_raw( '//instagram.com/' . $image['username'] ) : '',
							'caption'	=> array(
							...
							'user'		=> array(
												'username'	=> ! empty( $image['username'] ) ? sanitize_text_field( $image['username'] ) : '',
												'id' 		=> $user_id,
												),
					...
					foreach ( $images as $image ) {
						...
						$thumbs = array(
							'standard_resolution'	=> array(
															'url'    => preg_replace( "/^https:/i", "", esc_url_raw( $image['thumbnail_src'] ) ),
															'width'  => ! empty( $image['dimensions']['width'] ) ? sanitize_text_field( $image['dimensions']['width'] ) : '',
															'height' => ! empty( $image['dimensions']['height'] ) ? sanitize_text_field( $image['dimensions']['height'] ) : '',
															),
							'small_resolution'		=> array(
															'url'    => preg_replace( "/^https:/i", "", esc_url_raw( $image['thumbnail_resources'][0]['src'] ) ),
															'width'  => ! empty( $image['thumbnail_resources'][0]['config_width'] ) ? sanitize_text_field( $image['thumbnail_resources'][0]['config_width'] ) : '',
															'height' => ! empty( $image['thumbnail_resources'][0]['config_height'] ) ? sanitize_text_field( $image['thumbnail_resources'][0]['config_height'] ) : '',
															),
							'medium_resolution'		=> array(
															'url'    => preg_replace( "/^https:/i", "", esc_url_raw( $image['thumbnail_resources'][2]['src'] ) ),
															'width'  => ! empty( $image['thumbnail_resources'][2]['config_width'] ) ? sanitize_text_field( $image['thumbnail_resources'][2]['config_width'] ) : '',
															'height' => ! empty( $image['thumbnail_resources'][2]['config_height'] ) ? sanitize_text_field( $image['thumbnail_resources'][2]['config_height'] ) : '',
															),
							'large_resolution'		=> array(
															'url'    => preg_replace( "/^https:/i", "", esc_url_raw( $image['thumbnail_resources'][4]['src'] ) ),
															'width'  => ! empty( $image['thumbnail_resources'][4]['config_width'] ) ? sanitize_text_field( $image['thumbnail_resources'][4]['config_width'] ) : '',
															'height' => ! empty( $image['thumbnail_resources'][4]['config_height'] ) ? sanitize_text_field( $image['thumbnail_resources'][4]['config_height'] ) : '',
															),
						);
						...
						$instagram[] = array(
							'type'		=> $type,
							'link'		=> esc_url_raw( '//instagram.com/p/' . $image['shortcode'] ),
							'caption'	=> array(
												'text' => ! empty( $image['edge_media_to_caption']['edges'][0]['node']['text'] )
															? sanitize_text_field( $image['edge_media_to_caption']['edges'][0]['node']['text'] )
															: esc_html__( 'Instagram Image', 'trx_addons' )
												),
							'user'		=> array(
												'username'	=> ! empty( $image['owner']['username'] ) ? sanitize_text_field( $image['owner']['username'] ) : '',
												'id' 		=> ! empty( $image['owner']['id'] ) ? sanitize_text_field( $image['owner']['id'] ) : '',
												),
							'comments'	=> array( 'count' => sanitize_text_field( $image['edge_media_to_comment']['count'] ) ),
							'likes'		=> array( 'count' => sanitize_text_field( $image['edge_liked_by']['count'] ) ),
							'images'	=> $thumbs,
						);
