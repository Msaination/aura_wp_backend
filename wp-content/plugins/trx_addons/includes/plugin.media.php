...
if ( ! function_exists( 'trx_addons_allow_upload_glb' ) ) {
	add_filter( 'upload_mimes', 'trx_addons_allow_upload_glb' );
	/**
	 * Allow to upload GLB 3D models
	 * 
	 * @hooked upload_mimes
	 * 
	 * @param  array $mime_types Mime types.
	 ...
	 * @return array             Mime types.
	 */
	function trx_addons_allow_upload_glb( $mime_types ) {
		if ( trx_addons_get_setting( 'allow_upload_glb', false ) ) {
...
//			'parallax' => 0,				// Parallax shift (in %)
			...
			'height_extra' => '',			// Slides height extra from the Elementor settings
		...
		if ( empty($images) || ! is_array($images) ) {
			...
			foreach ( $images as $k=>$v ) {
				...
				$images[$k]['button_link_atts'] = trx_addons_get_link_attributes( $v, 'button_link' );
			}
		}
		...
		if ( is_array( $images ) && count( $images ) > 0 ) {
						...
						. ( (int)$args['height'] > 0 && ( trx_addons_is_on( $args['noresize'] ) || $args['noresize'] == 1 )
							? ' style="'
								. ( $args['height'] == '100vh'
									...
									: ( empty( $args['height_extra'] ) ? esc_attr( trx_addons_get_css_position_from_values( '', '', '', '', '', $args['height'] ) ) : '' )	// Don't add height for the slider in Elementor widget, because the height is set by the widget in CSS
									)
								. '"'
							: '' )
						. '>'
			...
			foreach ( $images as $image ) {
				...
				if ( ! trx_addons_is_off( $args['titles'] ) ) {
					...
					$with_button_link = ! empty( $image['button_caption'] ) && ! empty( $image['button_link'] );
					if ( empty( $titles_content ) ) {
						...
						if ( ! empty( $image['date'] ) ) {
							$titles_content .= '<div class="slide_date">' . trim( nl2br( $image['date'] ) ) . '</div>';
						}
						if ( $with_button_link ) {
							$titles_content .= '<div class="slide_button">'
													. '<a href="' . esc_url( $image['button_link'] ) . '"'
														. ( ! empty( $image['button_link_atts'] ) ? $image['button_link_atts'] : '' )
														. ' class="sc_button_default"'
														. '>'
															. trim( $image['button_caption'] )
													. '</a>'
												. '</div>';
						}
						if ( ! empty( $titles_content ) ) {
					...
					if ( ! empty( $titles_content ) ) {
						$titles = '<div class="slide_info slide_info_' . ( trx_addons_is_on( $args['large'] ) ? 'large' : 'small' ) . ( $with_button_link ? ' slide_info_with_button' : '' ) . '">' 
										. trim( $titles_content ) 
									. '</div>';
...
if ( ! function_exists( 'trx_addons_get_embed_video' ) ) {
	...
	function trx_addons_get_embed_video( $video, $use_wp_embed = false, $args = array() ) {
		...
		if ( $use_wp_embed && is_object( $wp_embed ) ) {
			...
			$embed_video  = '<iframe'
								. ' src="'
									. ( strpos( $video, 'instagram.com' ) !== false
										? esc_url_raw( $video )		// esc_url() is damage video url from Instagram
										: esc_url( $video )
										)
									. '"'
								//. ' allow="autoplay"'
