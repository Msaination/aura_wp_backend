	...
	function trx_addons_get_list_terms( $prepend_inherit = false, $taxonomy = 'category', $opt = array() ) {
		...
		if ( empty( $list[ $hash ] ) ) {
			...
			foreach( (array)$taxonomy as $t ) {
				$hierarchical = is_taxonomy_hierarchical( $t );
				if ( $hierarchical ) {
					break;
				}
			}
...
if ( ! function_exists( 'trx_addons_get_hierarchical_terms' ) ) {
	...
	function trx_addons_get_hierarchical_terms( $terms, $parent = 0, $level = 0 ) {
		// Group terms by parent once. The previous version rescanned the whole $terms array
		// for every term (O(n^2)) - prohibitive on lists with tens of thousands of terms;
		// with the parent map the walk below is linear.
		$children = array();
		foreach ( $terms as $term ) {
			$term_parent = empty( $term->parent ) ? 0 : $term->parent;
			$children[ $term_parent ][] = $term;
		}
		$walk = function( $parent, $level ) use ( &$walk, &$children ) {
			$list = array();
			...
			foreach ( $children[ $parent ] as $term ) {
				...
				$list[] = $term;
				foreach ( $walk( $term->term_id, $level + 1 ) as $t ) {
					$list[] = $t;
				}
			}
			return $list;
		};
		return $walk( $parent, $level );
	}
}
...
if ( ! function_exists( 'trx_addons_get_hierarchical_list' ) ) {
	...
	function trx_addons_get_hierarchical_list($terms, $parent=0, $level=0, $key='id') {
		// Group terms by parent once. The previous version rescanned the whole $terms array
		// for every term (O(n^2)) - prohibitive on taxonomies with tens of thousands of terms
		// (~8.5s for 50K terms); with the parent map the walk below is linear.
		$children = array();
		foreach ( $terms as $term ) {
			$term_parent = empty( $term->parent ) ? 0 : $term->parent;
			$children[ $term_parent ][] = $term;
		}
		$walk = function( $parent, $level ) use ( &$walk, &$children, $key ) {
			$list = array();
			if ( empty( $children[ $parent ] ) ) {
				return $list;
			}
			foreach ( $children[ $parent ] as $term ) {
				...
				foreach ( $walk( $term->term_id, $level + 1 ) as $k => $v ) {
					$list[ $k ] = $v;
				}
			}
			return $list;
...
if ( ! function_exists( 'trx_addons_get_list_ease' ) ) {
	 ...
	 */
	function trx_addons_get_list_ease( $prepend_inherit = false, $type = 'gsap' ) {
		if ( $type == 'gsap' ) {
			...
			$list = apply_filters( 'trx_addons_filter_get_list_ease', array(
				'linear' => esc_html__( 'Linear', 'trx_addons' ),
				'swing'  => esc_html__( 'Swing', 'trx_addons' ),
				'easeInQuad' => esc_html__( 'Ease In Quad', 'trx_addons' ),
				'easeOutQuad' => esc_html__( 'Ease Out Quad', 'trx_addons' ),
				'easeInOutQuad' => esc_html__( 'Ease In Out Quad', 'trx_addons' ),
				'easeInCubic' => esc_html__( 'Ease In Cubic', 'trx_addons' ),
				'easeOutCubic' => esc_html__( 'Ease Out Cubic', 'trx_addons' ),
				'easeInOutCubic' => esc_html__( 'Ease In Out Cubic', 'trx_addons' ),
				'easeInQuart' => esc_html__( 'Ease In Quart', 'trx_addons' ),
				'easeOutQuart' => esc_html__( 'Ease Out Quart', 'trx_addons' ),
				'easeInOutQuart' => esc_html__( 'Ease In Out Quart', 'trx_addons' ),
				'easeInQuint' => esc_html__( 'Ease In Quint', 'trx_addons' ),
				'easeOutQuint' => esc_html__( 'Ease Out Quint', 'trx_addons' ),
				'easeInOutQuint' => esc_html__( 'Ease In Out Quint', 'trx_addons' )
			), $type );
		}
		return $prepend_inherit 
					? trx_addons_array_merge( array( 'inherit' => esc_html__( 'Inherit', 'trx_addons' ) ), $list ) 
...
if ( ! function_exists( 'trx_addons_get_list_sc_slider_titles' ) ) {
	...
	function trx_addons_get_list_sc_slider_titles() {
		return apply_filters( 'trx_addons_filter_get_list_sc_slider_titles', array(
			...
			'center'       => esc_html__( 'Center', 'trx_addons' ),
			'center_left'  => esc_html__( 'Center Left', 'trx_addons' ),
			'center_right' => esc_html__( 'Center Right', 'trx_addons' ),
