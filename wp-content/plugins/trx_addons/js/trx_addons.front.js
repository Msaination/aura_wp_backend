...
//							autoKill: true,
					...
					.on('click', 'a[href*="#"]', function(e) {
						...
						if ( trx_addons_is_local_link(href) ) {
							...
							if ( pos >= 0 ) {
								...
								var link_target = jQuery(href).eq(0);
								if ( link_target.length == 0 ) {
									return true;
								}
								// Skin any non-menu-anchor link if the plugin setting 'animate_inner_links' is off
								if ( TRX_ADDONS_STORAGE['animate_inner_links'] < 1 ) {
									if ( link_target.data( 'anchor-animation-offset' ) === undefined
										&& ( link_target.data( 'anchor-animation-ease' ) === undefined
											|| link_target.data( 'anchor-animation-ease' ) == 'none'
											)
									) {
										return true;
									}
								...
								trx_addons_document_animate_to( link_target );
								e.preventDefault();
								return false;
							}
						}
			...
			// or Magnific Popup Engine
				...
				window.trx_addons_prepare_popup_content = function(container, autoplay) {
					...
					if (wrapper.data('popup-content') === undefined) {
						...
						// Remove video and audio atomic players to reinit them
						wrapper.find('[data-e-type="e-youtube"] iframe,[data-e-type="e-vimeo"] iframe').remove();
		...
		var trx_addons_set_active_menu_items = trx_addons_throttle( function() {
			const screenMiddle = trx_addons_window_scroll_top() + Math.floor( trx_addons_window_height() / 3.3 );	// 30% from the window top
