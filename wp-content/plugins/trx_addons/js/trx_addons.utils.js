...
//						}
	...
	// Get the current scroll position of the document or scroll it to the specified position
	// Attention! The option 'behavior: instant' disables a native smooth scroll of the browser.
	//            The value 'auto' can't be used here, because it means "use a value of the CSS property
	//            'scroll-behavior' of the element" and not "scroll immediately".
	window.trx_addons_document_scroll_top = function( pos ) {
		var el = document.scrollingElement || document.documentElement;
		if ( pos === undefined ) {
			return window.pageYOffset !== undefined ? window.pageYOffset : el.scrollTop;
		}
		if ( trx_addons_browser_is_support_scroll_options() ) {
			window.scrollTo( { top: pos, left: window.pageXOffset || 0, behavior: 'instant' } );
		} else {
			el.scrollTop = pos;
			if ( document.body && el !== document.body ) {
				document.body.scrollTop = pos;
			}
		}
	};

	// Disable a native smooth scroll (a CSS rule "scroll-behavior: smooth", applied to the html or body
	// by the theme, a page builder or a third-party plugin) while our own scroll animation is running.
	// Without it Chromium-based browsers (Chrome, Edge, Opera, etc.) start own smooth scroll on each change
	// of the scroll position, restart it on the next frame and, as a result, the page stands still
	// until our animation is finished and jumps to the final position after that
	// (so a height of the sticky rows, appeared while scrolling, is not taken into account).
	var _scroll_behavior_saved = null;
	window.trx_addons_document_scroll_behavior_override = function() {
		if ( _scroll_behavior_saved !== null ) {
			return;
		}
		_scroll_behavior_saved = [];
		jQuery( 'html,body' ).each( function() {
			_scroll_behavior_saved.push( {
				el: this,
				value: this.style.getPropertyValue( 'scroll-behavior' ),
				priority: this.style.getPropertyPriority( 'scroll-behavior' )
			} );
			// Use 'important' to override a CSS rule with the same priority
			this.style.setProperty( 'scroll-behavior', 'auto', 'important' );
		} );
	};

	// Restore an original value of the CSS property 'scroll-behavior'
	window.trx_addons_document_scroll_behavior_restore = function() {
		if ( _scroll_behavior_saved === null ) {
			return;
		}
		for ( var i = 0; i < _scroll_behavior_saved.length; i++ ) {
			var item = _scroll_behavior_saved[i];
			if ( item.value === '' ) {
		...
		_scroll_behavior_saved = null;
	};

	// Request the next animation frame (with a fallback to the setTimeout for the very old browsers)
	window.trx_addons_animation_frame = function( callback ) {
		var raf = trx_addons_request_animation_frame();
		return raf !== null
				? raf.call( window, callback )
				: setTimeout( callback, 16 );
	};

	// Cancel the previously requested animation frame
	window.trx_addons_cancel_animation_frame = function( frame ) {
	...
	window.trx_addons_document_animate_stop = function() {
		if ( window.trx_addons_document_animate_to_frame ) {
			trx_addons_cancel_animation_frame( window.trx_addons_document_animate_to_frame );
			window.trx_addons_document_animate_to_frame = 0;
		}
		trx_addons_document_scroll_behavior_restore();
		// Stop an animation, started by the third-party scripts
	...
	window.trx_addons_document_animate_to_busy = false;
	window.trx_addons_document_animate_to_frame = 0;
	window.trx_addons_document_animate_to = function(id, callback, force) {
		...
		var data = {
			...
			delay: 0,
			offset: 0,
			offset_from: 'fixed_rows'	// 'fixed_rows' or 'top'
		};
		if ( isNaN(id) ) {
			// Detect object by id or class
			if ( typeof id == 'object' ) {
				if ( id.length > 0 ) {
					obj = id;
				} else {
			...
			// Get data-parameters from the object
			if ( obj.data( 'anchor-animation-ease' ) !== undefined ) {
				data.ease = obj.data( 'anchor-animation-ease' );
			}
			if ( obj.data( 'anchor-animation-duration' ) !== undefined ) {
				data.duration = obj.data( 'anchor-animation-duration' );
			}
			if ( obj.data( 'anchor-animation-delay' ) !== undefined ) {
				data.delay = obj.data( 'anchor-animation-delay' );
				if ( data.delay > 0 && ! force ) {
					setTimeout( function() {
						window.trx_addons_document_animate_to( id, callback, true );
					}, data.delay );
					return;
				}
			}
			if ( obj.data( 'anchor-animation-offset' ) !== undefined ) {
				// An offset can be different for each device (breakpoint),
				// so get a value for the current window width
				data.offset = parseInt( trx_addons_get_responsive_data_value( obj, 'anchor-animation-offset', 0 ), 10 );
				if ( isNaN( data.offset ) ) {
					data.offset = 0;
				}
			}
			if ( obj.data( 'anchor-animation-offset-from' ) !== undefined ) {
				data.offset_from = obj.data( 'anchor-animation-offset-from' );
			}
			// Calculate offset
			oft = Math.max( 0, obj.offset().top - ( data.offset_from === 'fixed_rows' ? trx_addons_fixed_rows_height() : 0 ) - data.offset );
		}
		var speed = data.duration !== ''
					? data.duration
					: Math.min(1000, Math.max(300, Math.round(Math.abs( oft - jQuery(window).scrollTop()) / jQuery(window).height() * 300)));
		if ( data.offset_from == 'fixed_rows' ) {
			if ( speed < 300 ) {
				speed = 300;
			}
			if ( data.ease == 'none' ) {
		...
		// Stop a previous animation (if any)
		trx_addons_document_animate_stop();
		// Disable a native smooth scroll of the browser while our animation is running
		trx_addons_document_scroll_behavior_override();
		window.trx_addons_document_animate_to_busy = true;
		if ( speed > 0 ) {
			var duration = Math.max( 100, speed ),
				start_pos = trx_addons_document_scroll_top(),
				start_time = 0,
				// If a required easing is not found (the script 'jquery-easing' is not loaded) -
				// use a default jQuery easing 'swing' (as the jQuery.animate() does)
				ease = jQuery.easing === undefined
						? null
						: ( typeof jQuery.easing[ data.ease ] == 'function'
							? jQuery.easing[ data.ease ]
							: ( data.ease != 'linear' && typeof jQuery.easing.swing == 'function' ? jQuery.easing.swing : null )
							),
				// Recalculate a target position on each frame, because sticky rows may stick (or unstick)
				// while the page is scrolling, so their total height (and the target position) is changed
				get_target = function() {
					var pos = obj !== null && data.offset_from === 'fixed_rows'
								? obj.offset().top - trx_addons_fixed_rows_height() - data.offset
								: oft;
					// Don't try to scroll over the last available position
					return Math.max( 0, Math.min( pos, Math.max( 0, trx_addons_document_height() - trx_addons_window_height() ) ) );
				},
				finish = function( pos ) {
					window.trx_addons_document_animate_to_frame = 0;
					trx_addons_document_scroll_top( pos );
					trx_addons_document_scroll_behavior_restore();
					_window_scroll_top = $window.scrollTop();
					window.trx_addons_document_animate_to_busy = false;
				...
				step = function() {
					...
					var progress = Math.min( 1, ( now - start_time ) / duration ),
						target = get_target();
					if ( progress >= 1 ) {
						finish( target );
						return;
					}
					// Signature of the easing functions from the plugin 'jquery.easing': ( x, t, b, c, d )
					var factor = ease !== null ? ease( progress, duration * progress, 0, 1, duration ) : progress;
					trx_addons_document_scroll_top( Math.round( start_pos + ( target - start_pos ) * factor ) );
					window.trx_addons_document_animate_to_frame = trx_addons_animation_frame( step );
				};
			window.trx_addons_document_animate_to_frame = trx_addons_animation_frame( step );
		} else {
			trx_addons_document_scroll_top( oft );
			trx_addons_document_scroll_behavior_restore();
			_window_scroll_top = $window.scrollTop();
			window.trx_addons_document_animate_to_busy = false;
			if ( callback ) callback( id, speed );
	...
	// Check if the browser supports an object (instead of the numbers) as a parameter of the window.scrollTo()
	// (all browsers with a support of the CSS property 'scroll-behavior' support the scroll options too)
	window.trx_addons_browser_is_support_scroll_options = function() {
		return 'scrollBehavior' in document.documentElement.style && typeof window.scrollTo == 'function';
	};

	// Check if the browser is based on the Blink engine (Chrome, Edge, Opera, Brave, Yandex, Vivaldi, etc.)
	// Such browsers restart own smooth scroll on each change of the scrollTop,
	// if a CSS rule "scroll-behavior: smooth" is applied to the html (or body)
	window.trx_addons_browser_is_blink = function() {
		return !! window.chrome && !! window.CSS && navigator.userAgent.indexOf( 'Chrome' ) >= 0;
	};

	// Check if the browser is Firefox
	window.trx_addons_browser_is_firefox = function() {
		return navigator.userAgent.indexOf( 'Firefox' ) >= 0;
	};

	// Check if a native smooth scroll (a CSS rule "scroll-behavior: smooth") is applied to the document
	window.trx_addons_browser_is_native_smooth_scroll = function() {
		var el = document.scrollingElement || document.documentElement;
		return window.getComputedStyle !== undefined
				&& window.getComputedStyle( el ).scrollBehavior === 'smooth';
	};
	...
	// Return a value of the data-parameter for the current window width.
	// If the object has an additional parameter "data-<name>-breakpoints" with pairs "max_width": "value" -
	// a value for the first breakpoint, that is greater or equal to the current window width, is returned,
	// otherwise - a value of the parameter "data-<name>" is returned.
	window.trx_addons_get_responsive_data_value = function( obj, name, default_value ) {
		var value = obj.data( name ) !== undefined ? obj.data( name ) : default_value,
			bp = obj.data( name + '-breakpoints' );
		if ( bp && typeof bp == 'object' ) {
			var ww = trx_addons_window_width();
			// Attention! Keys of the object are sorted in the ascending order by the JS engine,
			//            because they are integer-like strings
			for ( var max in bp ) {
				if ( ww <= Number( max ) ) {
					if ( bp[ max ] !== '' && bp[ max ] !== null ) {
						value = bp[ max ];
					}
					break;
				}
