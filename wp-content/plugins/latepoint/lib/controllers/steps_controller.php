...
if ( ! class_exists( 'OsStepsController' ) ) :
	...
	class OsStepsController extends OsController {
		...
		public function start_instant() {
			$atts = [];
			if ( ! empty( $this->params['selected_agent'] ) ) {
				$atts['selected_agent'] = sanitize_text_field( $this->params['selected_agent'] );
			}
			if ( ! empty( $this->params['selected_service'] ) ) {
				$atts['selected_service'] = sanitize_text_field( $this->params['selected_service'] );
			}
			if ( ! empty( $this->params['selected_location'] ) ) {
				$atts['selected_location'] = sanitize_text_field( $this->params['selected_location'] );
			}

			if ( ! empty( $this->params['hide_side_panel'] ) && $this->params['hide_side_panel'] == 'yes' ) {
				$atts['hide_side_panel'] = 'yes';
			}
			if ( ! empty( $this->params['hide_summary'] ) && $this->params['hide_summary'] == 'yes' ) {
				$atts['hide_summary'] = 'yes';
			}
		...
		public function load_datepicker_month() {
			...
			$target_date_string = sanitize_text_field( $this->params['target_date_string'] ?? '' );
			if ( ! preg_match( '/^(\d{4})-(\d{1,2})-1$/', $target_date_string, $date_parts ) || ! checkdate( (int) $date_parts[2], 1, (int) $date_parts[1] ) ) {
				wp_send_json(
					[
						'status'  => LATEPOINT_STATUS_ERROR,
						'message' => __( 'The calendar date is invalid. Please reload the booking form and try again.', 'latepoint' ),
					]
				);
				return;
			}
