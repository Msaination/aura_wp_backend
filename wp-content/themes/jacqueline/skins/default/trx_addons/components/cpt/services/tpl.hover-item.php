...
?>>
	...
	<div class="sc_services_item_content <?php echo 1 == (int)$args['hide_bg_image'] ? ' without_image' : ''; ?>"<?php if ((int)$args['hide_bg_image']==0 && !empty($image)) echo ' style="background-image: url('.esc_url($image).');"'; ?>>
		...
		<div class="sc_services_item_content_inner">
			...
			if ($args['featured'] != 'none') {
				if ($args['featured']=='icon' && !empty($meta['icon'])) {
					...
					?><img src="<?php echo esc_url($meta['image']); ?>" alt="<?php esc_attr_e('Icon', 'jacqueline'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
					echo !empty($link) 
						? '</a>' 
						: '</span>';
