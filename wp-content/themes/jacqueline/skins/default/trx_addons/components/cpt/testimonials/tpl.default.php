...
if ((int)$query->post_count > 0) {
	$posts_count = ($args['count'] > $query->post_count) ? $query->post_count : $args['count'];
	$args['columns'] = (int)$args['columns'] < 1 ? $posts_count : min($args['columns'], $posts_count);
		...
		while ( $query->have_posts() ) { $query->the_post();
			if ($args['slider'] && !empty($args['slider_pagination_thumbs'])) {
				...
				if('fashion' == $args['type']) {
					...
					$title_go = '<span class="sc_testimonials_item_author_title">'.get_the_title().'</span>';
