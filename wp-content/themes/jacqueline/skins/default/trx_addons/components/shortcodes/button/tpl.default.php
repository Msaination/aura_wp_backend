		...
		?><a href="<?php echo esc_url($item['link']); ?>"<?php
			...
			if (!empty($item['image']) || !empty($item['icon'])) {
				?><span class="sc_button_icon"><?php
					if (!empty($item['image'])) {
						...
						?><img class="sc_icon_as_image" src="<?php echo esc_url($item['image']); ?>" alt="<?php esc_attr_e('Icon', 'jacqueline'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
						...
						if (strpos($item['icon'], '.svg') !== false) {
							...
							?><img class="sc_icon_as_image" src="<?php echo esc_url($item['icon']); ?>" alt="<?php esc_attr_e('Icon', 'jacqueline'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
						}
					} else {
			...
			if (!empty($item['title']) || !empty($item['subtitle'])) {
				?><span class="sc_button_text<?php if (!trx_addons_is_off($item['text_align'])) echo ' sc_align_'.esc_attr($item['text_align']); ?>"><?php
					...
					if (!empty($item['title'])) {
						if ( $item['type'] == 'modern' ) {
                        	?><span class="sc_button_title_first"><?php echo esc_html($item['title']); ?></span><span class="sc_button_title_second"><?php echo esc_html($item['title']); ?></span><?php
						} else {
