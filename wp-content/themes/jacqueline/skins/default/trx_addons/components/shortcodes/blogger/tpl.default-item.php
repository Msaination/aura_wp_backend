...
if ( isset($templates[$args['type']][$args['template_'.$args['type']]]) ) {
		...
		if ( substr( $k, 0, 4 ) == 'meta') {
			...
			if ( false && (
				...
				if(!empty($title)) {
					...
					echo jacqueline_strwords($title, 13);
					jacqueline_show_layout((!empty($post_link) ? '</a>' : '') . '</h6>');
				}
			} else {
	...
	?><div data-post-id="<?php the_ID(); ?>" data-item-number="<?php echo esc_attr($args['item_number']); ?>" <?php
		post_class(
			...
			. ' sc_blogger_item_'.esc_attr((int)$args['item_number'] % 2 == 0 ? 'even' : 'odd')
		...
		?><div class="sc_blogger_item_body"><?php
			...
			if ( isset($template['layout']['featured']) ) {
				...
				$featured_args = array(
									...
									'thumb_ratio' => $args['image_ratio'] == 'none' ? ((int)$args['columns'] > 2 ? '71:53': '840:473') : $args['image_ratio'],
