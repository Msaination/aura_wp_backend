...
?><div <?php if (!empty($args['id'])) echo ' id="'.esc_attr($args['id']).'"'; ?> 
	...
	foreach ($args['prices'] as $item) {
		...
		?>>
			...
			if (!empty($item['image'])) {
				$item['image'] = trx_addons_get_attachment_url($item['image'], apply_filters('trx_addons_filter_thumb_size', trx_addons_get_thumb_size('avatar'), 'price-default'));
				if (!empty($item['image'])) {
					$attr = trx_addons_getimagesize($item['image']);
					?><div class="sc_price_item_image">
                        <img src="<?php echo esc_url($item['image']); ?>" alt="<?php esc_attr_e('Icon', 'jacqueline'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
                    	if (!empty($item['price'])) {
				...
				if (!empty($icon)) {
					...
					?><div class="sc_price_item_icon sc_price_item_icon_type_<?php echo esc_attr($item['icon_type']); ?> <?php echo esc_attr($icon); ?>"
						...
						if (!empty($svg)) {
							...
							?><img class="sc_icon_as_image" src="<?php echo esc_url($img); ?>" alt="<?php esc_attr_e('Icon', 'jacqueline'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
						} else {
			...
			?><div class="sc_price_item_info"><?php
                ...
                if (!empty($item['price']) && empty($item['image'])) {
                    ?><div class="sc_price_item_price"><?php
                    if (!empty($item['before_price'])) {
                        ?><span class="sc_price_item_price_before"><?php echo wp_kses(trx_addons_parse_codes($item['before_price']), 'trx_addons_kses_content'); ?></span><?php
                    }
