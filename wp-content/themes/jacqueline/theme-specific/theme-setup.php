 ...
 *
 * @package JACQUELINE
 * @since JACQUELINE 1.0.22
 */

// If this theme is a free version of premium theme
if ( ! defined( 'JACQUELINE_THEME_FREE' ) ) {
	define( 'JACQUELINE_THEME_FREE', false );
}
if ( ! defined( 'JACQUELINE_THEME_FREE_WP' ) ) {
	define( 'JACQUELINE_THEME_FREE_WP', false );
}

// If this theme support external updates
if ( ! defined( 'JACQUELINE_THEME_ALLOW_UPDATE' ) ) {
	define( 'JACQUELINE_THEME_ALLOW_UPDATE', true );
}

// If this theme uses skins
if ( ! defined( 'JACQUELINE_ALLOW_SKINS' ) ) {
	define( 'JACQUELINE_ALLOW_SKINS', true );
}
if ( ! defined( 'JACQUELINE_DEFAULT_SKIN' ) ) {
	define( 'JACQUELINE_DEFAULT_SKIN', 'default' );
}
if ( ! defined( 'JACQUELINE_REMEMBER_SKIN' ) ) {
	define( 'JACQUELINE_REMEMBER_SKIN', false );
}
...
//-------------------------------------------------------------------------
$GLOBALS['JACQUELINE_STORAGE'] = array(
...
if ( ! function_exists( 'jacqueline_get_theme_pro_key' ) ) {
	/**
	 ...
	 */
	function jacqueline_get_theme_pro_key() {
		$theme_pro_key = get_option( sprintf( 'purchase_code_src_%s', get_template() ) );
		if ( empty( $theme_pro_key ) ) {
			$theme_pro_key = jacqueline_storage_get( 'theme_pro_key' );
		}
		return $theme_pro_key;
	}
}

if ( ! function_exists( 'jacqueline_trx_updater_get_theme_info' ) ) {
	add_filter( 'trx_updater_filter_get_theme_info', 'jacqueline_trx_updater_get_theme_info' );
	/**
	 ...
	 */
	function jacqueline_trx_updater_get_theme_info( $info ) {
		$engine = jacqueline_storage_get( 'engine' );
...
if ( ! function_exists( 'jacqueline_original_theme_slug' ) ) {
	add_filter( 'jacqueline_filter_original_theme_slug', 'jacqueline_original_theme_slug' );
	/**
	 ...
	 *
	 * @hooked 'jacqueline_filter_original_theme_slug'
	 *
	 * @param string $theme_slug  Current slug of the theme.
	 *
	 * @return string             Original slug of the theme.
	 */
	function jacqueline_original_theme_slug( $theme_slug = '' ) {
		return 'jacqueline';
	}
}
...
if ( ! function_exists( 'jacqueline_importer_set_options' ) ) {
	add_filter( 'trx_addons_filter_importer_options', 'jacqueline_importer_set_options', 9 );
	/**
	...
	function jacqueline_importer_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			...
			// Prepare demo data
			$options['demo_url'] = esc_url( jacqueline_get_protocol() . ':' . jacqueline_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
			$options['required_plugins'] = array_keys( jacqueline_storage_get( 'required_plugins' ) );
...
if ( ! function_exists( 'jacqueline_ocdi_set_options' ) ) {
	add_filter( 'trx_addons_filter_ocdi_options', 'jacqueline_ocdi_set_options', 9 );
	/**
	 ...
	 * @return array          Processed options with a theme-specific values.
	 */
	function jacqueline_ocdi_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			// Prepare demo data
			$options['demo_url'] = esc_url( jacqueline_get_protocol() . ':' . jacqueline_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
...
if ( ! function_exists( 'jacqueline_customizer_theme_setup1' ) ) {
	add_action( 'after_setup_theme', 'jacqueline_customizer_theme_setup1', 1 );
	/**
	 ...
	 */
	function jacqueline_customizer_theme_setup1() {
		jacqueline_storage_set(
			'settings', array(
				...
				'allow_fullscreen'          => false,                   // Allow cases 'fullscreen' and 'fullwide' (if a next settings is true) for the body style in the Theme Options
																		...
																		// (can be removed if filter 'jacqueline_filter_allow_fullscreen' return false)
