...
if ( ! function_exists( 'jacqueline_wp_theme_setup' ) ) {

	add_action( 'after_setup_theme', 'jacqueline_wp_theme_setup' );
	 ...
	 */
	function jacqueline_wp_theme_setup() {

		// Remove macros from title
		add_filter( 'wp_title', 'jacqueline_remove_macros_from_site_title' );
		add_filter( 'wp_title_parts', 'jacqueline_remove_macros_from_site_title' );
		add_filter( 'document_title_parts', 'jacqueline_remove_macros_from_site_title' );

		// Breadcrumbs link 'All posts'
		add_filter( 'post_type_archive_link', 'jacqueline_get_template_page_link', 10, 2 );
	}
}


if ( ! function_exists( 'jacqueline_is_preview' ) ) {
	/**
	 ...
	 */
	function jacqueline_is_preview( $builder = 'any' ) {
		return ( in_array( $builder, array( 'any', 'elm', 'elementor' ) ) && function_exists( 'jacqueline_elementor_is_preview' ) && jacqueline_elementor_is_preview() )
				||
				( in_array( $builder, array( 'any', 'gb', 'gutenberg' ) ) && function_exists( 'jacqueline_gutenberg_is_preview' ) && jacqueline_gutenberg_is_preview() );
	}
}


if ( ! function_exists( 'jacqueline_get_wp_template_hooks' ) ) {
	/**
	 ...
	 */
	function jacqueline_get_wp_template_hooks() {
		return apply_filters( 'jacqueline_filter_wp_template_hooks', array(
					'404_template',
					'archive_template',
					'attachment_template',
					'author_template',
...
if ( ! function_exists( 'jacqueline_detect_blog_mode' ) ) {
	/**
	 ...
	 * 
	 * For example:
	 *   $pos = jacqueline_get_theme_option( 'sidebar_position' );
	 * 
	 ...
	 */
	function jacqueline_detect_blog_mode() {
		if ( is_front_page() && ! is_home() ) {
			...
			$mode = '404';
		} elseif ( jacqueline_is_single() ) {
			$mode = 'post';
		} elseif ( is_page() && ! jacqueline_storage_isset( 'blog_archive' ) ) {
			$mode = 'page';
		} elseif ( is_category() ) {
			$mode = 'category';
		} elseif ( is_tag() ) {
		...
		return apply_filters( 'jacqueline_filter_detect_blog_mode', $mode );
	}
}

if ( ! function_exists( 'jacqueline_is_blog_mode_custom' ) ) {
	/**
	 ...
	 */
	function jacqueline_is_blog_mode_custom() {
		return ! in_array( jacqueline_storage_get( 'blog_mode' ), apply_filters( 'jacqueline_filter_blog_mode_standard', array(
...
if ( ! function_exists( 'jacqueline_get_current_mode_image' ) ) {
	/**
	 ...
	 */
	function jacqueline_get_current_mode_image( $default = '' ) {
		if ( is_category() || is_tax() ) {
			$img = jacqueline_get_term_image();
			if ( '' != $img ) {
				$default = $img;
			}
		} elseif ( jacqueline_is_singular() || jacqueline_storage_isset( 'blog_archive' ) ) {
			if ( is_home() ) {
		...
		return jacqueline_clear_thumb_size( $default );
	}
}

if ( ! function_exists( 'jacqueline_blog_archive_start' ) ) {
	/**
	 ...
	 */
	function jacqueline_blog_archive_start() {
		$main_post = jacqueline_storage_get( 'blog_archive_template_post' );
		if ( is_object( $main_post ) ) {
			...
			$jacqueline_content            = '';
			$jacqueline_blog_archive_mask  = '%%CONTENT%%';
			$jacqueline_blog_archive_subst = sprintf( '<div class="blog_archive">%s</div>', $jacqueline_blog_archive_mask );
			$jacqueline_content            = apply_filters( 'the_content', get_the_content() );
			...
			if ( '' != $jacqueline_content ) {
				$jacqueline_pos = stripos( $jacqueline_content, $jacqueline_blog_archive_mask );
				if ( false !== $jacqueline_pos ) {
					$jacqueline_content = preg_replace( '/(\<p\>\s*)?' . $jacqueline_blog_archive_mask . '(\s*\<\/p\>)/i', $jacqueline_blog_archive_subst, $jacqueline_content );
				} else {
					$jacqueline_content .= $jacqueline_blog_archive_subst;
				}
				$jacqueline_content = explode( $jacqueline_blog_archive_mask, $jacqueline_content );
				// Display first part
				jacqueline_show_layout( apply_filters( 'jacqueline_filter_blog_archive_start', $jacqueline_content[0] ) );
				// And store second part
				jacqueline_storage_set( 'blog_archive_end', $jacqueline_content[1] );
			}
...
if ( ! function_exists( 'jacqueline_blog_archive_end' ) ) {
	/**
	 * Display the end blog archive template.
	 * 
	 * Output a second part (stored in the previous call of jacqueline_blog_archive_start).
	 */
	function jacqueline_blog_archive_end() {
		$html = jacqueline_storage_get( 'blog_archive_end' );
		if ( '' != $html ) {
			// Display second part of template content
			jacqueline_show_layout( apply_filters( 'jacqueline_filter_blog_archive_end', $html ) );
		}
	}
}

if ( ! function_exists( 'jacqueline_blog_archive_get_template' ) ) {
	/**
	 ...
	 */
	function jacqueline_blog_archive_get_template( $blog_style = '' ) {
		if ( empty( $blog_style ) ) {
			$blog_style = jacqueline_get_theme_option( 'blog_style' );
		}
		$parts   = explode( '_', $blog_style );
		$archive = 'index';
		if ( jacqueline_storage_isset( 'blog_styles', $parts[0], 'archive' ) ) {
			$archive = jacqueline_storage_get_array( 'blog_styles', $parts[0], 'archive' );
		}
		return apply_filters( 'jacqueline_filter_blog_archive_template', $archive, $blog_style );
	}
}

if ( ! function_exists( 'jacqueline_blog_item_get_template' ) ) {
	/**
	 ...
	 *                           or an empty string if not detected.
	 */
	function jacqueline_blog_item_get_template( $blog_style = '' ) {
		if ( empty( $blog_style ) ) {
			$blog_style = jacqueline_get_theme_option( 'blog_style' );
		}
		$blog_style = jacqueline_sanitize_slug( $blog_style );
		$parts = explode( '_', $blog_style );
		$item  = '';
		if ( strpos( $parts[0], 'blog-custom-' ) === 0 ) {
			$item = 'templates/content-custom';
		} elseif ( jacqueline_storage_isset( 'blog_styles', $parts[0], 'item' ) ) {
			$item = jacqueline_storage_get_array( 'blog_styles', $parts[0], 'item' );
		} else {
...
if ( ! function_exists('jacqueline_get_the_ID') ) {
	/**
	 * Return current post ID
	 * 
	 * @return int  Current post ID
	 */
	function jacqueline_get_the_ID() {
		global $wp_query;
		return in_the_loop() 
					? get_the_ID() 
					: ( ! empty( $wp_query->post->ID )
						? $wp_query->post->ID
						: ( jacqueline_is_singular() && ! empty( $wp_query->queried_object->ID )
...
if ( ! function_exists( 'jacqueline_get_post_id' ) ) {
	/**
	 ...
	 */
	function jacqueline_get_post_id( $args = array() ) {
		$args  = array_merge(
			array(
				'posts_per_page' => 1,
			), $args
...
if ( ! function_exists( 'jacqueline_get_post_content' ) ) {
	/**
	 ...
	 */
	function jacqueline_get_post_content( $apply_filters = false ) {
		global $post;
...
if ( ! function_exists( 'jacqueline_filter_post_content' ) ) {
	/**
	 ...
	 */
	function jacqueline_filter_post_content( $content ) {
		$content = apply_filters( 'jacqueline_filter_post_content', $content );
		global $wp_embed;
...
if ( ! function_exists( 'jacqueline_get_template_page_id' ) ) {
	/**
	 ...
	 */
	function jacqueline_get_template_page_id( $args = array() ) {
		$args   = array_merge(
			array(
				'template'   => 'blog.php',
				'post_type'  => 'post',
		...
		if ( ! empty( $args['post_type'] ) ) {
			$q_args['meta_query'][] = array(
				'key'     => 'jacqueline_options_post_type',
		...
		if ( '' !== $args['parent_cat'] ) {
			$q_args['meta_query'][] = array(
				'key'     => 'jacqueline_options_parent_cat',
				'value'   => $args['parent_cat'] > 0 ? $args['parent_cat'] : 1,
				'compare' => $args['parent_cat'] > 0 ? '=' : '<',
			);
		}
		return jacqueline_get_post_id( $q_args );
	}
}

if ( ! function_exists( 'jacqueline_get_template_page_link' ) ) {
	/**
	 ...
	 * 
	 * Handler of the add_filter('post_type_archive_link', 'jacqueline_get_template_page_link', 10, 2 );
	 * 
	 ...
	 */
	function jacqueline_get_template_page_link( $link = '', $post_type = '' ) {
		if ( ! empty( $post_type ) ) {
			$id = jacqueline_get_template_page_id(
				array(
					'post_type'  => $post_type,
					'parent_cat' => 0,
				)
...
if ( ! function_exists( 'jacqueline_get_posts_archive_template' ) ) {

	add_filter( 'archive_template', 'jacqueline_get_posts_archive_template', 100 );
	 ...
	 */
	function jacqueline_get_posts_archive_template( $template ) {
		if ( jacqueline_get_theme_option( 'use_blog_archive_pages' ) ) {
			if ( is_post_type_archive() ) {
				$obj = get_queried_object();
				if ( ! empty( $obj->name ) ) {
					$templates = get_option( 'jacqueline_blog_archive_templates' );
					if ( ! empty( $templates[ $obj->name ] ) ) {
						$template = jacqueline_redirect_to_archive_template( $template, $templates[ $obj->name ] );
					}
				}
			} else {
				$template = jacqueline_get_tax_archive_template( $template );
			}
		}
		return $template;
	}	
...
if ( ! function_exists( 'jacqueline_get_tax_archive_template' ) ) {

	add_filter( 'category_template', 'jacqueline_get_tax_archive_template', 100 );
	add_filter( 'taxonomy_template', 'jacqueline_get_tax_archive_template', 100 );
	 ...
	 * @return string          A filtered path to the template
	 */
	function jacqueline_get_tax_archive_template( $template ) {
		if ( jacqueline_get_theme_option( 'use_blog_archive_pages' ) && ( is_category() || is_tag() || is_tax() ) ) {
			$obj = get_queried_object();
			global $wp_query;
			...
			if ( $pt && $tax && $term ) {
				$pt_tax  = jacqueline_get_post_type_taxonomy( $pt );
				if ( $pt_tax == $tax ) {
					$templates = get_option( 'jacqueline_blog_archive_templates' );
					if ( ! empty( $templates[ "{$pt}_{$tax}_{$term}" ] ) ) {
						$template = jacqueline_redirect_to_archive_template( $template, $templates[ "{$pt}_{$tax}_{$term}" ] );
					} else {
						$found = false;
						do {
							...
							if ( ! empty( $templates[ "{$pt}_{$tax}_{$parent}" ] ) ) {
								$template = jacqueline_redirect_to_archive_template( $template, $templates[ "{$pt}_{$tax}_{$parent}" ], $term );
								$found = true;
								break;
							} else {
						...
						if ( ! $found && ! empty( $templates[ "{$pt}" ] ) ) {
							$template = jacqueline_redirect_to_archive_template( $template, $templates[ "{$pt}" ], $term );
						}
					}
				}
			}
...
if ( ! function_exists( 'jacqueline_redirect_to_archive_template' ) ) {
	/**
	 ...
	 */
	function jacqueline_redirect_to_archive_template( $template, $page_id, $term = false ) {
		// Store page number
		...
		// Load page options
		jacqueline_override_theme_options( null, $page_id );
		// Override parent category
		if ( is_object( $term ) ) {
			$term = ! empty( $term->term_id ) ? $term->term_id : 0;
		}
		if ( $term > 0 ) {
			jacqueline_storage_set_array( 'options_meta', 'parent_cat', $term );
		}
		return jacqueline_get_file_dir( 'blog.php' );
	}
}

if ( ! function_exists( 'jacqueline_save_archive_template' ) ) {

	add_action( 'save_post', 'jacqueline_save_archive_template', 11 );
	 ...
	 */
	function jacqueline_save_archive_template( $post_id ) {
		// verify nonce
		if ( ! wp_verify_nonce( jacqueline_get_value_gp( 'override_options_nonce' ), admin_url() ) ) {
			return $post_id;
		}
		...
		// Check permissions
		if ( empty( $post_id ) || ! jacqueline_current_user_can_edit( $post_id, $post_type, 'page' ) ) {
			return $post_id;
		}
		...
		if ( 'page' == $post_type ) {
			...
			if ( 'blog.php' == $page_template ) {
				...
				$tax  = jacqueline_get_post_type_taxonomy( $pt );
				$term = isset( $meta['parent_cat'] ) ? $meta['parent_cat'] : 0;
				$templates = jacqueline_array_delete_by_value( $templates, $post_id );
				$templates[ $pt . ( $term > 0 ? "_{$tax}_{$term}" : '' ) ] = $post_id;
			} else {
				$templates = jacqueline_array_delete_by_value( $templates, $post_id );
			}
			update_option( 'jacqueline_blog_archive_templates', $templates );
		}
	}
}

// Delete page from templates
if ( ! function_exists( 'jacqueline_delete_archive_template' ) ) {

	add_action( 'delete_post', 'jacqueline_delete_archive_template', 11 );
	 ...
	 */
	function jacqueline_delete_archive_template( $post_id ) {
		$templates = get_option( 'jacqueline_blog_archive_templates' );
		if ( is_array( $templates ) ) {
			$templates = jacqueline_array_delete_by_value( $templates, $post_id );
			update_option( 'jacqueline_blog_archive_templates', $templates );
...
if ( ! function_exists( 'jacqueline_redirect_404' ) ) {
	add_action( 'template_redirect', 'jacqueline_redirect_404', 1 );
	/**
	 * Redirect to the 404 page if the page is selected in the theme options.
	 * 
	 * @hooked template_redirect
	 */
	function jacqueline_redirect_404() {
		if ( is_404() ) {
			$page_id = jacqueline_get_theme_option( 'redirect_404_page', '' );
			if ( ! jacqueline_is_off( $page_id ) ) {
				$way = apply_filters( 'jacqueline_filter_redirect_404_way', 'show' );	// show | redirect
				if ( $way == 'redirect' ) {
					...
					if ( is_object( $post ) && ! empty( $post->ID ) && $post->ID == $page_id ) {
						...
						jacqueline_override_theme_options();
					}
				}
			}
		}
...
if ( ! function_exists( 'jacqueline_get_protocol' ) ) {
	/**
	 * Return a current protocol ( http or https ) of the site.
	 * 
	 * @return string A string with a protocol.
	 */
	function jacqueline_get_protocol( $suffix = false ) {
		return ( is_ssl() ? 'https' : 'http' ) . ( ! empty( $suffix ) ? ':' : '' );
	}
}

if ( ! function_exists( 'jacqueline_get_hash_link' ) ) {
	/**
	 ...
	 */
	function jacqueline_get_hash_link( $hash ) {
		if ( 0 !== strpos( $hash, 'http' ) ) {
			if ( '#' != $hash[0] ) {
				$hash = '#' . $hash;
			}
			if ( is_customize_preview() ) {
				$url = jacqueline_get_current_url();
				$pos = strpos( $url, '#' );
				if ( false !== $pos ) {
...
if ( ! function_exists( 'jacqueline_get_current_url' ) ) {
	/**
	 * Return an URL to the current page.
	 * 
	 * @return string  A current page URL.
	 */
	function jacqueline_get_current_url() {
		return add_query_arg( array() );
	}
}

if ( ! function_exists( 'jacqueline_check_url' ) ) {
	/**
	 ...
	 */
	function jacqueline_check_url( $val ) {
		if ( ! is_array( $val ) ) {
			$val = array( $val );
		}
		$rez = false;
...
if ( ! function_exists( 'jacqueline_attachment_url_to_postid' ) ) {
	/**
	 ...
	 */
	function jacqueline_attachment_url_to_postid( $url ) {
		static $images = array();
		if ( ! isset( $images[ $url ] ) ) {
			$images[ $url ] = attachment_url_to_postid( jacqueline_clear_thumb_size( $url, false ) );
		}
		return $images[ $url ];
	}
}

if ( ! function_exists( 'jacqueline_remove_macros_from_site_title' ) ) {
	/**
	 ...
	 */
	function jacqueline_remove_macros_from_site_title( $title ) {
		if ( is_array( $title ) ) {
			foreach ( $title as $k => $v ) {
				$title[ $k ] = jacqueline_remove_macros( $v );
			}
		} else {
			$title = jacqueline_remove_macros( $title );
		}
		return $title;
	}
}

if ( ! function_exists( 'jacqueline_get_blog_title' ) ) {
	/**
	 * Return a blog title for the current blog mode.
	 *
	 * @return string  A blog title.
	 */
	function jacqueline_get_blog_title() {

		if ( is_front_page() ) {
			$title = esc_html__( 'Home', 'jacqueline' );
		} elseif ( is_home() ) {
			$title = get_option( 'page_for_posts' ) == get_queried_object_id()
						? get_the_title( get_queried_object_id() )
						: esc_html__( 'All Posts', 'jacqueline' );
		} elseif ( is_author() ) {
			...
			$title = sprintf( esc_html__( 'Daily Archives: %s', 'jacqueline' ), get_the_date() );
		} elseif ( is_month() ) {
			// Translators: Add the queried month to the title
			$title = sprintf( esc_html__( 'Monthly Archives: %s', 'jacqueline' ), get_the_date( 'F Y' ) );
		} elseif ( is_year() ) {
			...
			$title = sprintf( esc_html__( 'Attachment: %s', 'jacqueline' ), get_the_title() );
		} elseif ( jacqueline_is_single() || is_page() ) {
			$title = get_the_title();
		} else {
			$title = get_the_title();
		}
		return apply_filters( 'jacqueline_filter_get_blog_title', $title );
	}
}

if ( ! function_exists( 'jacqueline_get_post_categories' ) ) {
	/**
	 ...
	 */
	function jacqueline_get_post_categories( $delimiter = ', ', $id = false, $links = true ) {
		return jacqueline_get_post_terms( $delimiter, $id, 'category', $links );
	}
}

if ( ! function_exists( 'jacqueline_get_post_terms' ) ) {
	/**
	 ...
	 */
	function jacqueline_get_post_terms( $delimiter = ', ', $id = false, $taxonomy = 'category', $links = true ) {
		$output = '';
		if ( empty( $id ) ) {
			$id = get_the_ID();
		}
		if ( empty( $taxonomy ) ) {
			$taxonomy = jacqueline_get_post_type_taxonomy( get_post_type( $id ) );
		}
				...
				$output .= ( ! empty( $output )
							...
							. ( $links
								? '<a href="' . esc_url( get_term_link( $term->term_id, $taxonomy ) ) . '"'
										...
										. ' title="' . sprintf( esc_attr__( 'View all posts in %s', 'jacqueline' ), $term->name ) . '"'
										. '>'
								: '<span>'
								)
								. apply_filters( 'jacqueline_filter_term_name', $term->name, $term )
							. ( $links
								? '</a>'
								: '</span>'
								);
...
if ( ! function_exists( 'jacqueline_get_post_type_taxonomy' ) ) {
	/**
	 ...
	 */
	function jacqueline_get_post_type_taxonomy( $post_type = '' ) {
		if ( empty( $post_type ) ) {
		...
		return apply_filters( 'jacqueline_filter_post_type_taxonomy', $tax, $post_type );
	}
}

if ( ! function_exists( 'jacqueline_get_edited_post_id' ) ) {
	/**
	 ...
	 */
	function jacqueline_get_edited_post_id() {
		$id = false;
		if ( is_admin() ) {
			$url = jacqueline_get_current_url();
			if ( strpos( $url, 'post.php' ) !== false ) {
				if ( jacqueline_get_value_gp( 'action' ) == 'edit' ) {
					$post_id = jacqueline_get_value_gp( 'post' );
					if ( 0 < $post_id ) {
						$id = $post_id;
					}
				}
...
if ( ! function_exists( 'jacqueline_get_edited_post_type' ) ) {
	/**
	 ...
	 */
	function jacqueline_get_edited_post_type() {
		$pt = '';
		if ( is_admin() ) {
			$url = jacqueline_get_current_url();
			if ( strpos( $url, 'post.php' ) !== false ) {
				if ( in_array( jacqueline_get_value_gp( 'action' ), array( 'edit', 'elementor' ) ) ) {
					$id = jacqueline_get_value_gp( 'post' );
					if ( 0 < $id ) {
				...
				$pt = jacqueline_get_value_gp( 'post_type' );
			}
		}
		return $pt;
	}
...
if ( ! function_exists( 'jacqueline_is_post_edit' ) ) {
	/**
	 * Detect if a current mode is "Edit post" or "Site editor".
	 *
	 * @return bool Return true if current mode is "Edit post" or "Edit site".
	 */
	function jacqueline_is_post_edit() {
		return ( jacqueline_check_url( 'post.php' ) && ! empty( $_GET['action'] ) && $_GET['action'] == 'edit')
				||
				jacqueline_check_url( 'post-new.php' )
				||
				( jacqueline_check_url( '/block-renderer/trx-addons/' ) && ! empty( $_GET['context'] ) && $_GET['context'] == 'edit' )
				||
				( jacqueline_check_url( 'admin.php' ) && ! empty( $_GET['page'] ) && $_GET['page'] == 'gutenberg-edit-site' )
				||
				( jacqueline_check_url( 'site-editor.php' ) && ! empty( $_GET['postType'] ) )	// || $_GET['postType'] == 'wp_template' ) )
				||
				jacqueline_check_url( 'widgets.php' );
	}
}

if ( ! function_exists( 'jacqueline_is_singular' ) ) {
	/**
	 ...
	 */
	function jacqueline_is_singular( $type = '' ) {
		global $wp_query;
		return apply_filters( 'jacqueline_filter_is_singular', ! empty( $wp_query->queried_object->ID ) && is_singular( $type ), $type );
	}
}

if ( ! function_exists( 'jacqueline_is_single' ) ) {
	/**
	 * Detect if the current post is a single post.
	 *
	 * @return bool  true if the current mode is a single post.
	 */
	function jacqueline_is_single() {
		global $wp_query;
		return apply_filters( 'jacqueline_filter_is_single', ! empty( $wp_query->queried_object->ID ) && is_single() );
	}
}

if ( ! function_exists( 'jacqueline_add_seo_itemprops' ) ) {
	/**
	 * Add SEO params to the article tag.
	 */
	function jacqueline_add_seo_itemprops() {
		if ( jacqueline_is_on( jacqueline_get_theme_option( 'seo_snippets' ) ) ) {
			...
			?>" 
			itemtype="<?php echo esc_attr( jacqueline_get_protocol( true ) ); ?>//schema.org/<?php echo esc_attr( jacqueline_get_markup_schema() ); ?>" 
...
if ( ! function_exists( 'jacqueline_add_seo_snippets' ) ) {
	/**
	 * Add SEO meta to the post from the template 'templates/seo.php'
	 */
	function jacqueline_add_seo_snippets() {
		if ( jacqueline_is_on( jacqueline_get_theme_option( 'seo_snippets' ) ) ) {
...
if ( ! function_exists( 'jacqueline_get_nav_menu' ) ) {
	/**
	 ...
	 * 									  or true to use a custom walker from the theme with class 'jacqueline_custom_menu_walker'.
	 * 									  Default is false.
	 *
	 * @return string               A menu layout or an error message.
	 */
	function jacqueline_get_nav_menu( $location = '', $menu = '', $depth = 0, $custom_walker = false ) {
		static $list = array();
		$slug = $location . '_' . $menu;
		if ( empty( $list[ $slug ] ) ) {
			$list[ $slug ] = esc_html__( 'You are trying to use a menu inserted in himself!', 'jacqueline' );
			$args          = array(
								'menu'            => empty( $menu ) || 'default' == $menu || jacqueline_is_inherit( $menu ) ? '' : $menu,
								'container'       => '',
								'container_class' => '',
								'container_id'    => '',
								...
								'before'          => '',
								'after'           => '',
								'link_before'     => jacqueline_get_theme_setting( 'wrap_menu_items_with_span' ) ? '<span>' : '',
								'link_after'      => jacqueline_get_theme_setting( 'wrap_menu_items_with_span' ) ? '</span>' : '',
								'depth'           => $depth,
							);
			if ( ! empty( $location ) ) {
				$args['theme_location'] = $location;
			} else if ( empty( $args['theme_location'] ) && JACQUELINE_THEME_FREE_WP ) {
				$args['theme_location'] = 'menu_main';
			}
			if ( $custom_walker ) {
				if ( is_object( $custom_walker ) ) {
					$args['walker'] = $custom_walker;
				} else if ( $custom_walker === true && class_exists( 'jacqueline_custom_menu_walker' ) ) {
					$args['walker'] = new jacqueline_custom_menu_walker;
				}
			}
			$list[ $slug ] = wp_nav_menu( apply_filters( 'jacqueline_filter_get_nav_menu_args', $args ) );
		}
		return apply_filters( 'jacqueline_filter_get_nav_menu', $list[ $slug ], $location, $menu );
	}
}

if ( ! function_exists( 'jacqueline_remove_empty_spaces_between_menu_items' ) ) {

    add_action( 'wp_nav_menu', 'jacqueline_remove_empty_spaces_between_menu_items', 98, 2 );
	 ...
	 */
	function jacqueline_remove_empty_spaces_between_menu_items( $html = '', $args = array() ) {
		return preg_replace(
							array( "/>[\r\n\s]*<li/", "/>[\r\n\s]*<\\/ul>/" ),
							array( "><li", "></ul>" ),
							$html
...
if ( ! function_exists( 'jacqueline_remove_empty_menu_items' ) ) {

	add_action( 'wp_nav_menu', 'jacqueline_remove_empty_menu_items', 99, 2 );
	 ...
	 * @return string       A clean string.
	 */
	function jacqueline_remove_empty_menu_items( $html = '', $args = array() ) {
		return jacqueline_get_theme_setting( 'remove_empty_menu_items' )
					? preg_replace(
							"/<li[^>]*>[\r\n\s]*<a[^>]*>[\r\n\s]*(<span>[\r\n\s]*<\\/span>[\r\n\s]*)?<\\/a>[\r\n\s]*<\\/li>/",
							"",
							$html
...
//
if ( ! function_exists( 'jacqueline_new_main_query' ) ) {
	/**
	 ...
	 */
	function jacqueline_new_main_query( $args ) {
		...
		if ( ! empty( $args['post_type'] ) || ! empty( $args['category'] ) ) {
			$query_args  = jacqueline_query_add_posts_and_cats( $query_args, $args['post_ids'], $args['post_type'], $args['category'] );
		}
		if ( $args[ 'page' ] > 1 ) {
...
if ( ! function_exists( 'jacqueline_query_add_sort_order' ) ) {
	/**
	 ...
	 */
	function jacqueline_query_add_sort_order( $args, $orderby = 'date', $order = 'desc' ) {
		if ( ! empty( $orderby ) && ( empty( $args['orderby'] ) || 'none' != $orderby ) ) {
			$q          = apply_filters( 'jacqueline_filter_add_sort_order', array(), $orderby, $order );
		...
		return apply_filters( 'jacqueline_filter_add_sort_order_args', $args, $orderby, $order );
	}
}

if ( ! function_exists( 'jacqueline_query_add_posts_and_cats' ) ) {
	/**
	 ...
	 */
	function jacqueline_query_add_posts_and_cats( $args, $ids = '', $post_type = '', $cat = '', $taxonomy = '' ) {
		if ( ! empty( $ids ) ) {
			...
			if ( ! empty( $cat ) ) {
				...
				if ( empty( $taxonomy ) ) {
					$taxonomy = jacqueline_get_post_type_taxonomy( $post_type );
				}
...
if ( ! function_exists( 'jacqueline_query_add_filters' ) ) {
	/**
	 ...
	 */
	function jacqueline_query_add_filters( $args, $filters = false ) {
		if ( ! empty( $filters ) ) {
			if ( ! is_array( $filters ) ) {
...
if ( ! function_exists( 'jacqueline_create_widgets_area' ) ) {
	/**
	 ...
	 */
	function jacqueline_create_widgets_area( $name, $add_classes = '' ) {
		$widgets_name = jacqueline_get_theme_option( $name, 'hide' );
		if ( ! jacqueline_is_off( $widgets_name ) && is_active_sidebar( $widgets_name ) ) {
			jacqueline_storage_set( 'current_sidebar', $name );
			ob_start();
			...
			if ( ! empty( $out ) ) {
				...
				$need_columns = strpos( $out, 'columns_wrap' ) === false && apply_filters( 'jacqueline_filter_widgets_area_need_columns', 'footer' == $name, $name, $out );
				if ( $need_columns ) {
					$columns = apply_filters( 'jacqueline_filter_widgets_area_columns', min( 4, max( 1, jacqueline_tags_count( $out, 'aside' ) ) ), $name );
				...
				<div class="<?php echo esc_attr( $name ); ?> <?php echo esc_attr( $name ); ?>_wrap widget_area">
					<?php do_action( 'jacqueline_action_before_sidebar_wrap' ); ?>
					<div class="<?php echo esc_attr( $name ); ?>_inner <?php echo esc_attr( $name ); ?>_inner widget_area_inner">
						<?php
						do_action( 'jacqueline_action_before_sidebar' );
						jacqueline_show_layout(
							$out,
							true == $need_columns ? '<div class="columns_wrap">' : '',
							true == $need_columns ? '</div>' : ''
						);
						do_action( 'jacqueline_action_after_sidebar' );
						?>
					</div>
					<?php do_action( 'jacqueline_action_after_sidebar_wrap' ); ?>
				</div>
				<?php
			}
		}
...
if ( ! function_exists( 'jacqueline_sidebar_present' ) ) {
	/**
	 ...
	 */
	function jacqueline_sidebar_present() {
		global $wp_query;
		$sidebar_position = jacqueline_get_theme_option( 'sidebar_position' );
		$sidebar_type     = jacqueline_get_theme_option( 'sidebar_type' );
		$sidebar_name     = jacqueline_get_theme_option( 'sidebar_widgets' );
		return apply_filters(
			'jacqueline_filter_sidebar_present',
			! jacqueline_is_off( $sidebar_position )
				&& (
						( 'default' == $sidebar_type && ! jacqueline_is_off( $sidebar_name ) && is_active_sidebar( $sidebar_name ) )
						||
						( 'custom' == $sidebar_type && jacqueline_is_layouts_available() )
					)
				&& ! is_404()
...
if ( ! function_exists( 'jacqueline_get_content_width' ) ) {
	/**
	 ...
	 */
	function jacqueline_get_content_width() {
		$pg_width = apply_filters( 'jacqueline_filter_content_width', (int)jacqueline_get_theme_option( 'page_width' ) );
		$sb_width = 0;
		$sb_gap   = 0;
		if ( jacqueline_sidebar_present() ) {
			...
			if ( 'default' == $jacqueline_sidebar_type ) {
				...
				if ( is_active_sidebar( $jacqueline_sidebar_name ) ) {
					$sb_width = (int)jacqueline_get_theme_option( 'sidebar_width' );
					$sb_gap   = (int)jacqueline_get_theme_option( 'sidebar_gap' );
				}
			} else {
				$sb_width = (int)jacqueline_get_theme_option( 'sidebar_width' );
				$sb_gap   = (int)jacqueline_get_theme_option( 'sidebar_gap' );
...
if ( ! function_exists( 'jacqueline_add_inline_css_class' ) ) {
	/**
	 ...
	 */
	function jacqueline_add_inline_css_class( $css, $suffix = '' ) {
		$class_name = jacqueline_generate_id( 'jacqueline_inline_' );
		jacqueline_add_inline_css(
			sprintf(
				'.%s%s{%s}',
				$class_name,
				! empty( $suffix )
...
if ( ! function_exists( 'jacqueline_add_inline_css' ) ) {
	/**
...
------------------------------------------------------------------------------------- */
if ( ! function_exists( 'jacqueline_enqueue_styles' ) ) {
	/**
	 * Enqueue a list of styles
	 *
	 * @param array $list - styles to enqueue
	 */
	function jacqueline_enqueue_styles( $list, $sc ) {
		if ( is_array( $list ) ) {
			foreach( $list as $handle => $data ) {
				$lib_url = jacqueline_get_file_url( $data['src'] );
				if ( $lib_url ) {
					wp_enqueue_style(
						$handle,
						$lib_url,
						...
						! empty( $data['media'] ) ? jacqueline_media_for_load_css_responsive( str_replace( '_', '-', $sc ), $data['media'] ) : 'all'
					);
				}
			}
		}
...
if ( ! function_exists( 'jacqueline_enqueue_scripts' ) ) {
	/**
	 * Enqueue a list of scripts
	 *
	 * @param array $list - scripts to enqueue
	 */
	function jacqueline_enqueue_scripts( $list, $sc ) {
		if ( is_array( $list ) ) {
			foreach( $list as $handle => $data ) {
				$lib_url = jacqueline_get_file_url( $data['src'] );
				if ( $lib_url ) {
...
if ( ! function_exists( 'jacqueline_enqueue_optimized' ) ) {
	/**
	 ...
	 */
	function jacqueline_enqueue_optimized( $sc, $force, $args ) {
		static $loaded = array();
		if ( empty( $loaded[ $sc ] ) && (
			current_action() == 'wp_enqueue_scripts' && jacqueline_need_frontend_scripts( ! empty( $args['slug'] ) ? $args['slug'] :  $sc )
			||
			...
			if ( ! isset( $args['need'] ) || $args['need'] ) {
				if ( ! empty( $args['css'] ) ) {
					jacqueline_enqueue_styles( $args['css'], $sc );
				}
				if ( ! empty( $args['js'] ) ) {
					jacqueline_enqueue_scripts( $args['js'], $sc );
				}
...
if ( ! function_exists( 'jacqueline_enqueue_optimized_responsive' ) ) {
	/**
	 ...
	 * @param array $args - arguments with styles and scripts to enqueue
	 */
	function jacqueline_enqueue_optimized_responsive( $sc, $force, $args ) {
		static $loaded = array();
		if ( empty( $loaded[ $sc ] ) && (
			current_action() == 'wp_enqueue_scripts' && jacqueline_need_frontend_scripts( ! empty( $args['slug'] ) ? $args['slug'] :  $sc )
			||
...
if ( ! function_exists( 'jacqueline_get_date' ) ) {
	/**
	 ...
	 */
	function jacqueline_get_date( $dt = '', $format = '', $disable_date_diff = false ) {
		$diff   = $disable_date_diff ? 0 : intval( jacqueline_get_theme_option( 'time_diff_before' ) );
		...
		if ( $diff == 0 || $dt_cur - $dt > $diff * 24 * 3600 ) {
			...
			$dt = sprintf( esc_html__( '%s ago', 'jacqueline' ), human_time_diff( $dt, $dt_cur ) );
		}
		return $dt;
	}
}
...
if ( ! function_exists( 'jacqueline_lazy_load_off' ) ) {
	/**
	 * Disable a lazy load behaviour for rest images on the current page
	 */
	function jacqueline_lazy_load_off() {
		if ( ! jacqueline_lazy_load_is_off() ) {
			add_filter( 'wp_lazy_loading_enabled', 'jacqueline_lazy_load_disabled' );
		}
	}
}

if ( ! function_exists( 'jacqueline_lazy_load_disabled' ) ) {
	/**
	 ...
	 */
	function jacqueline_lazy_load_disabled() {
		return false;
	}
}

if ( ! function_exists( 'jacqueline_lazy_load_on' ) ) {
	/**
	 * Enable a lazy load behaviour for rest images on the current page
	 */
	function jacqueline_lazy_load_on() {
		if ( jacqueline_lazy_load_is_off() ) {
			remove_action( 'wp_lazy_loading_enabled', 'jacqueline_lazy_load_disabled' );	// Is equal to rf
		}
	}
}

if ( ! function_exists( 'jacqueline_lazy_load_is_off' ) ) {
	/**
	 * Return a current state of the lazy load behaviour
	 *
	 * @return bool  A current lazy load state
	 */
	function jacqueline_lazy_load_is_off() {
		return has_filter( 'wp_lazy_loading_enabled', 'jacqueline_lazy_load_disabled' );
	}
}
...
if ( ! function_exists( 'jacqueline_get_markup_schema' ) ) {
	/**
	 * Return a structured data markup schema for the current page.
	 *
	 * @return string  A schema of the current page.
	 */
	function jacqueline_get_markup_schema() {
		if ( jacqueline_is_single() ) {                                        // Is single post
			$type = 'Article';
...
if ( ! function_exists( 'jacqueline_get_privacy_text' ) ) {
	/**
	 * Return a text for the Privacy Policy checkbox.
	 *
	 * @return string  A label text.
	 */
	function jacqueline_get_privacy_text() {
		return apply_filters( 'jacqueline_filter_privacy_text', wp_kses( trim( jacqueline_get_theme_option( 'privacy_text' ) ), 'jacqueline_kses_content' ) );
	}
}

if ( ! function_exists( 'jacqueline_add_privacy_page_link' ) ) {
	/**
	 * Adds the Privacy Policy page link to the Privacy text
	 * 
	 * @hooked filter jacqueline_filter_privacy_text
	 * 
	 * @param string $text - the text to modify
	 *
	 * @return string  A label text.
	 */
	function jacqueline_add_privacy_page_link( $text ) {
		if ( ! empty( $text ) ) {
			...
			if ( ! empty( $page ) ) {
				$text .=  ( ! in_array( substr( $text, -1 ), array( '.', '!' ) ) ? '.' : '' )
							...
							. sprintf(
								__( 'For further details on handling user data, see our %s.', 'jacqueline' ),
								'<a href="' . esc_url( get_permalink( $page ) ) . '"' . jacqueline_external_links_target() . '>' . __( 'Privacy Policy', 'jacqueline' ) . '</a>'
							);
			}
		}
		return $text;
...
if ( ! function_exists( 'jacqueline_kses_allowed_html' ) ) {
	add_filter( 'wp_kses_allowed_html', 'jacqueline_kses_allowed_html', 10, 2 );
	/**
	 * Return a theme-specific tags list with allowed attributes for using in the wp_kses() calls
	 * ( if $context is equal to 'jacqueline_kses_content' or 'trx_addons_kses_content' ).
	 *
	 ...
	 */
	function jacqueline_kses_allowed_html( $tags, $context ) {
		if ( in_array( $context, array( 'jacqueline_kses_content', 'trx_addons_kses_content' ) ) ) {
			$tags = array( 
...
if ( ! function_exists( 'jacqueline_verify_nonce' ) ) {
	/**
	 ...
	 */
	function jacqueline_verify_nonce( $nonce = 'nonce', $mask = '', $die = true ) {
		$rez = true;
		if ( empty( $mask ) ) {
			$mask = admin_url('admin-ajax.php');
		}
		if ( ! wp_verify_nonce( jacqueline_get_value_gp( $nonce ), $mask ) ) {
			$rez = false;
			if ( $die ) {
				jacqueline_forbidden();
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'jacqueline_exit' ) ) {
	/**
	 ...
	 */
	function jacqueline_exit( $message = '', $title = '', $code = 200 ) {
		wp_die( $message, $title, array( 'response' => $code, 'exit' => empty( $message ) && empty( $title ) ) );
	}
}

if ( ! function_exists( 'jacqueline_forbidden' ) ) {
	/**
	 ...
	 * @param string $title    Optional. Title to output while exit.
	 */
	function jacqueline_forbidden( $message = '', $title = '' ) {
		jacqueline_exit( $message, $title, 403 );
	}
}

if ( ! function_exists( 'jacqueline_ajax_response' ) ) {
	/**
	 * Send an ajax response and exit.
	 *
	 * @param mixed $response  A response to send to the client.
	 */
	function jacqueline_ajax_response( $response ) {
...
if ( ! function_exists( 'jacqueline_get_upgrade_url' ) ) {
	/**
	 ...
	 */
	function jacqueline_get_upgrade_url( $params = array() ) {
		$theme_folder = get_template();
		$theme_slug   = apply_filters( 'jacqueline_filter_original_theme_slug', $theme_folder );
		$default      = array(
							'action'     => '',
		...
		if ( ! empty( $params['action'] ) && ! in_array( $params['action'], array( 'info_skins' ) ) ) {
			$default['key']        = '';
			$default['src']        = jacqueline_get_theme_pro_key();
			$default['theme_name'] = wp_get_theme( $theme_folder )->get( 'Name' );
			$default['domain']     = jacqueline_remove_protocol_from_url( home_url(), true );
		...
		return jacqueline_add_to_url( trailingslashit( jacqueline_storage_get( 'theme_upgrade_url' ) ) . 'upgrade.php', $params );
	}
}

if ( ! function_exists( 'jacqueline_get_upgrade_data' ) ) {
	/**
	 ...
	 */
	function jacqueline_get_upgrade_data( $params = array() ) {
		$url = jacqueline_get_upgrade_url( $params );
		$result = function_exists( 'trx_addons_fgc' ) ? trx_addons_fgc( $url ) : jacqueline_fgc( $url );
		if ( is_serialized( $result ) ) {
			try {
				$result = jacqueline_unserialize( $result );
			} catch ( Exception $e ) {
			}
		}
		if ( ! isset( $result['error'] ) || ! isset( $result['data'] ) ) {
			$result = array(
				'error' => esc_html__( 'Unrecognized server answer!', 'jacqueline' ),
				'data'  => ''
			);
		}
		return $result;
...
if ( ! function_exists( 'jacqueline_allow_upload_archives' ) ) {
	/**
	 * Allow WordPress to upload archives before get an upgrade package from the upgrade server.
	 */
	function jacqueline_allow_upload_archives() {
...
if ( ! function_exists( 'jacqueline_disallow_upload_archives' ) ) {
	/**
	 * Disallow WordPress to upload archives after the upgrade package received.
	 */
	function jacqueline_disallow_upload_archives() {
...
if ( ! function_exists( 'jacqueline_current_user_can_edit' ) ) {
	/**
	 ...
	 */
	function jacqueline_current_user_can_edit( $post_id, $post_type = '', $capability = 'post' ) {
		if ( empty( $post_type ) ) {
