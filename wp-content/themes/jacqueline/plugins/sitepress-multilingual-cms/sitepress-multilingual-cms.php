...
if ( ! defined( 'JACQUELINE_WPML_REMOVE_TRANSLATED_OPTIONS' ) ) {
	define( 'JACQUELINE_WPML_REMOVE_TRANSLATED_OPTIONS', false );
}
...
if ( ! function_exists( 'jacqueline_wpml_make_theme_options_translatable' ) ) {
	add_action( 'after_setup_theme', 'jacqueline_wpml_make_theme_options_translatable', 3 );
	function jacqueline_wpml_make_theme_options_translatable() {
		if ( jacqueline_exists_wpml() ) {
			$translatable = apply_filters( 'jacqueline_filter_theme_options_translatable', array() );
			if ( is_array( $translatable ) ) {
				foreach( $translatable as $option_name ) {
					if ( jacqueline_storage_isset( 'options', $option_name ) ) {
						jacqueline_storage_set_array2( 'options', $option_name, 'translate', true );
					}
				}
			}
		}
...
if ( ! function_exists( 'jacqueline_wpml_theme_setup9' ) ) {
	...
	function jacqueline_wpml_theme_setup9() {
		if ( jacqueline_exists_wpml() ) {
			add_action( 'wp_enqueue_scripts', 'jacqueline_wpml_frontend_scripts', 1100 );
...
if ( ! function_exists( 'jacqueline_wpml_tgmpa_required_plugins' ) ) {
	...
	function jacqueline_wpml_tgmpa_required_plugins( $list = array() ) {
		if ( jacqueline_storage_isset( 'required_plugins', 'sitepress-multilingual-cms' ) && jacqueline_storage_get_array( 'required_plugins', 'sitepress-multilingual-cms', 'install' ) !== false && jacqueline_is_theme_activated() ) {
			$path = jacqueline_get_plugin_source_path( 'plugins/sitepress-multilingual-cms/sitepress-multilingual-cms.zip' );
			if ( ! empty( $path ) || jacqueline_get_theme_setting( 'tgmpa_upload' ) ) {
				$list[] = array(
					'name'     => jacqueline_storage_get_array( 'required_plugins', 'sitepress-multilingual-cms', 'title' ),
			...
			foreach ( $JACQUELINE_STORAGE['options'] as $k => $v ) {
				if ( ! empty( $v['translate'] ) ) {
					if ( isset( $values[ $k ] ) ) {   // If key not present - value was not changed
						...
						if ( JACQUELINE_WPML_REMOVE_TRANSLATED_OPTIONS ) {
							unset( $values[ $k ] );
						}
						$changed = true;
					}
...
if ( ! function_exists( 'jacqueline_wpml_customizer_control_js' ) ) {
	add_action( 'customize_controls_enqueue_scripts', 'jacqueline_wpml_customizer_control_js' );
	function jacqueline_wpml_customizer_control_js() {
		wp_enqueue_script(
			'jacqueline-sitepress-multilingual-cms-customizer',
			jacqueline_get_file_url( 'plugins/sitepress-multilingual-cms/sitepress-multilingual-cms-customizer.js' ),
...
if ( ! function_exists( 'jacqueline_wpml_options_add_scripts' ) ) {
	add_action( 'admin_enqueue_scripts', 'jacqueline_wpml_options_add_scripts' );
	function jacqueline_wpml_options_add_scripts() {
		$screen = function_exists( 'get_current_screen' ) ? get_current_screen() : false;
		if ( is_object( $screen ) && false !== strpos($screen->id, '_page_theme_options') ) {
			wp_enqueue_script(
				'jacqueline-sitepress-multilingual-cms-options',
				jacqueline_get_file_url( 'plugins/sitepress-multilingual-cms/sitepress-multilingual-cms-options.js' ),
				array( 'jquery' ), null, true
			);
		}
	}
...
if ( ! function_exists( 'jacqueline_wpml_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'jacqueline_wpml_frontend_scripts', 1100 );
	function jacqueline_wpml_frontend_scripts() {
		if ( jacqueline_is_on( jacqueline_get_theme_option( 'debug_mode' ) ) ) {
			$jacqueline_url = jacqueline_get_file_url( 'plugins/sitepress-multilingual-cms/sitepress-multilingual-cms.css' );
			if ( '' != $jacqueline_url ) {
				wp_enqueue_style( 'jacqueline-sitepress-multilingual-cms', $jacqueline_url, array(), null );
			}
		}
	}
}
...
// Merge custom styles
if ( ! function_exists( 'jacqueline_wpml_merge_styles' ) ) {
	//Handler of the add_filter( 'jacqueline_filter_merge_styles', 'jacqueline_wpml_merge_styles');
	function jacqueline_wpml_merge_styles( $list ) {
