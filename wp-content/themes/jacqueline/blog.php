...
if ( function_exists( 'jacqueline_elementor_is_preview' ) && jacqueline_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			jacqueline_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	jacqueline_new_main_query(
		array(
			'post_type'      => jacqueline_get_theme_option( 'post_type' ),
			'category'       => jacqueline_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'jacqueline_filter_get_template_part', jacqueline_blog_archive_get_template() ) );
}
